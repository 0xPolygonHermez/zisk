...
# ZISKVM Documentation

Welcome to the ZisK VM docs.

> Replace this section with your ZisK VM introduction.
...
- F: single-precision float instructions:
  ...
  - They are low-performance, so we recommend to use them as little as possible
- D: double-precision float instructions:
  - Supported through a software float library, since float operations are very complex to prove in zero-knowledge
  - They are low-performance, so we recommend to use them as little as possible
- C: compressed instructions:
  - These instructions take 16 bits of binary code instead of the regular 32 bits
  - The encoded size of the instruction has a benefit when executing RISC-V code in HW processors, since it improves the locality of the code: more code fits in every memory page, so less memory pages need to be read
...
- ZisK:
  - Supports 4 64-bit registers, named a, b c and flag
...
This way, you get the equivalent result but constraining the operation to just 3 registers a, b and c, and in that specific order, at the lower cost of adding a new a and b source (RISC-V registers) and a new c store (RISC-V registers).
...
- The **source of register a** can be any of the following:
  - Immediate: a value is constant for this instruction
  - Memory: a value is read from a memory address that is constant for this ZisK instruction
  - Register: a value is read from a RISC-V register which index is constant for this ZisK instruction
  - c: a value copied from register c, i.e. a is the result of the previously emulated instruction
  - Step: a value is the current step value
- The **source of register b** can be any of the following:
  - Immediate: b value is constant for this instruction
  - Memory: b value is read from a memory address that is constant for this ZisK instruction
  - Register: b value is read from a RISC-V register which index is constant for this ZisK instruction
  - Indirect memory: b value is read from memory using:
...
- The **storage of register c** can be any of the following:
  - Memory: c value is written to a memory address that is constant for this ZisK instruction
  - Register: c value is written to a RISC-V register which index is constant for this ZisK instruction
  - Indirect memory: c value is written to memory using:
    - as the memory address, the value of register a plus an offset that is constant for this ZisK instruction
    - as the value width, either 1, 2, 4 or 8 bytes, being this width value constant for this ZisK instruction
...
There has been a transformation of the classic 32 general purpose register RISC-V processor into a 4 specific purpose register ZisK processor, with very similar operations and much more zero-knowledge friendly.
...
RV64IMA instructions are 32-bit long and 32-bit aligned.  ZisK instructions are 8-bit aligned.  Most RV64IMA instructions can be transpiled to one single ZisK instruction, and they share the same ROM address.  However, atomic instructions require the combination of 2, 3 or even 4 ZisK instructions.  ZisK transpiler takes advantage of the fact that RISC-V instruction addresses are 32-bit aligned to add ZisK instructions in the contiguous non-32-bit aligned addresses.

RISC-V C extension instructions are 16-bit long and 16-bit aligned.  When using C extension, 16-bit and 32-bit instructions (from C and non-C RISC-V extensions) are combined in potentially any order, so in general they are 16-bit aligned.  Most C extension instructions require 1 single ZisK instruction to implement them, but given their 16-bit size the transpiler can use 2 ZisK instructions if required.
...
The resulting ZisK code could have this layout:
...
72-79: "ZisK instruction"
80-87: "ZisK instruction"
88-95: "- no code -"
...
152-159: "ZisK instruction"
160-167: "ZisK instruction"
168-191: "- no code -"
```

Note that the ZisK instructions do not have an actual bit length but they have been represented as 8-bit instructions since their address is just 8-bits aligned.
...
|0x80000000|0x88000000|ROM|
|0xA0000000|0xBFFFFFFF|RAM|


ZisK code is the result of the RISC-V code transpilation, but it also contains some additional ziskos code that implements some infrastructure tasks.  The following tables describes the different address ranges and their usage.
...
|0x1110|Program launcher code.  It writes initial values of ROM and RAM globals.  It saves some data and jumps to the first program instruction at 0x80000000.  Syscall trap handler, including the exit sequence to make output public and jump to the ROM_EXIT address.|
...
|Address|Usage|
|---|---|
|0xA0000000|RAM_ADDR or SYS_ADDR. ziskos RAM.|
|0xA0010000|OUTPUT_ADDR.  Program output RAM data.|
|0xA0030000|Program RAM.|
|0xBFFF0000|Float soft library RAM|
...
- Rust emulator:
  - Slow, but good for checking and analyzing the guest program
  - You can use the `ziskemu` program to transpile and emulate your ELF program file with a specific binary input file
  ...
  - `ziskemu` also can provide a lot of statistical information about the emulation that can help you optimize your program for ZK proof generation on ZisK; you can see the complete description of `ziskemu` functionality here (TODO: add link to complete documentation)
  - Tip: You can tell `cargo-zisk` to use ziskemu internally by using the `-l` parameter.  Note that this will slow-down the emulation phase significantly, but it will also reduce its memory requirements
- Assembly emulator:
  - Very fast!
  - This is the default emulator used by cargo-zisk
  - You can call `cargo-zisk rom-setup -e file.elf` in order to get the assembly code generated from the program ELF file, and compiled to get a program-specific emulator binary.
  - Alternatively, if rom setup has not been called before, it will be called automatically during the first `cargo-zisk` command, making it slower due to the emulator binary generation.
...
flowchart TD
    A(Rust program) -->|cargo-zisk build --release| B(RISC-V program.elf
    RV64IMA)
    B -->|ziskemu -e program.elf -i input.bin| C(**Rust emulator**
    Program analysis)
    B -->|cargo-zisk *command* -e program.elf -i input.bin| D(**Assembly emulator** on RAM
    Very fast!)
    B -->|cargo-zisk *command* -e program.elf -i input.bin -u| E(**Assembly emulator** on disk
    Fast, less memory)
    B -->|cargo-zisk *command* -e program.elf -i input.bin -l| F(**Rust emulator**
    Slow, even less memory)
```