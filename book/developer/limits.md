...
| RAM SIZE | 511.75 MB | 256 MB (ETH) | 50 % |
...
This limit consists on the maximum ROM address space size in bytes.  Another way to describe it is the maximum size of the program code and the program constant data, together.  In essence, we are referring to the program size.

Currently, ZisK maps the program ROM to the virtual address space between addresses 0x8000_0000 and 0x87FF_FFFF.  The remaining space until the address 0x8FFF_FFFF is used by the float library ROM.  This leaves an effective limit of 127 MB.
...
This limit consists on the maximum RAM address space size in bytes

ZisK uses the following RAM virtual address map:

| From | To | Usage |
|---|---|---|
| 0xA000\_0000 | 0xA000\_7FFF | memory-mapped GPR registers |
| 0xA000\_8000 | 0xA000\_FFFF | memory-mapped float registers |
| 0xA001\_0000 | 0xA002\_FFFF | output |
| 0xA003\_0000 | 0xBFFE\_FFFF | program RAM (including heap and stack) |
| 0xBFFF\_0000 | 0xBFFF\_FFFF | float library RAM address |

This leaves an effective program RAM limit of 511.75 MB.
...
This limit consists on the maximum size of the minimal trace, or memory operations trace size, in bytes.

The trace size can occupy the 16-bit address space from address 0xD000\_0000 to address 0x08\_CFFF\_FFFF, leaving a maximum trace size of 32 GB.
...
This limit consists on the maximum number of execution steps.

Currently, the maximum number of steps is 2^36 \= 68719476736 (64 G steps) \= 0x1000000000, as per PIL configuration.
...
This limit consists on the maximum number of program instructions.  In other words, it is the number of different instructions present in the program.
...
This limit consists on the minimum physical memory required in the prover or worker system to execute the program, when using the ZisK assembly emulator.
...
This limit consists on the minimum physical memory required in the prover or worker system to execute the program, when using the ZisK Rust emulator.

The current limit is 32 GB.

## MEM COUNT\&PLAN SLOTS

This limit consists on the number of slots allocated to calculate the memory count & plan.

The current limit is 6 GB.

An Ethereum client consumes up to 2 GB.  This is aproximately 33 % of the maximum.

If during the emulation this limit is reached, the following message is displayed (original trace code is shown):

```
ERROR: MemCounter: no free slots left for this thread(" << id << "). Increase MAX_SLOT_GB in state-machines/mem-cpp/cpp/mem\_config.hpp and recompile zisk.
```
