...
cfg_if! {
    if #[cfg(zisk_guest)] {
        use core::arch::asm;
        use crate::{ziskos_fcall, ziskos_fcall_param, zisklib::FCALL_SECP256R1_FN_INV_ID};
        #[cfg(not(feature = "inputcpy"))]
        use crate::ziskos_fcall_get;
        #[cfg(feature = "inputcpy")]
        use crate::ziskos_inputcpy;
    } else {
        use zisk_lib_c::secp256r1_fn_inv_c;
    }
}

/// Hints the multiplicative inverse of a field element in the scalar field of the Secp256r1 curve.
///
/// `fcall_secp256r1_fn_inv` operates on a 256-bit field element represented as an array of four
/// `u64` values, and returns the inverse as an array of four `u64` values.
///
/// ### Safety
///
...
pub fn fcall_secp256r1_fn_inv(
    ...
    #[cfg(not(zisk_guest))]
    {
        ...
        secp256r1_fn_inv_c(x, &mut result);
        #[cfg(feature = "hints")]
        {
