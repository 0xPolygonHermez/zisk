...
pub fn pairing_batch_bn254(
    ...
    let mut g1_points_ml: ScratchVec<[u64; 8]> = new_scratch_vec(num_points);
    let mut g2_points_ml: ScratchVec<[u64; 16]> = new_scratch_vec(num_points);
...
pub fn pairing_check_bn254(
    ...
    // Collect valid pairs
    let mut g1_valid: ScratchVec<[u64; 8]> = new_scratch_vec(g1_points.len());
    let mut g2_valid: ScratchVec<[u64; 16]> = new_scratch_vec(g2_points.len());
...
pub(crate) unsafe fn bn254_pairing_check_c(
    ...
    // Parse all pairs
    let mut g1_points: ScratchVec<[u64; 8]> = new_scratch_vec(num_pairs);
    let mut g2_points: ScratchVec<[u64; 16]> = new_scratch_vec(num_pairs);
