...
pub fn exp_by_x_cyclo_bn254(a: &[u64; 48]) -> [u64; 48] {
    ...
    // First bit is 1 and second bit is 0, so set c = a,
    // compress the input and compute the compressed square C(a²)
    result[0..8].copy_from_slice(a0);
    result[8..16].copy_from_slice(a2);
    result[16..24].copy_from_slice(a4);
    result[24..32].copy_from_slice(a1);
    result[32..40].copy_from_slice(a3);
    result[40..48].copy_from_slice(a5);
    let mut comp = compress_cyclo_bn254(&result);
    comp = square_cyclo_bn254(&comp);

    // Third bit is 0, so compute C(c⁴)
    comp = square_cyclo_bn254(&comp);

    // Fourth bit is 0, so compute C(c⁸)
    comp = square_cyclo_bn254(&comp);

    // Fifth bit is 1, so compute C(c¹⁶), decompress to obtain c¹⁶ and multiply by a
    comp = square_cyclo_bn254(&comp);
    let decomp = decompress_cyclo_bn254(&comp);
    result = mul_fp12_bn254(&result, &decomp); // TODO: Check if I need to flip coeffs

    for &bit in X_BIN_BE.iter().skip(5) {
...
pub fn square_cyclo_bn254(a: &[u64; 32]) -> [u64; 32] {
    ...
    // b5 = 2·(a5 + 3·B23)
    let mut b5 = mul_fp2_bn254(&b23, &[3, 0, 0, 0, 1, 0, 0, 0]);
