...
/// Computes the Miller loop for the BLS12-381 curve for a batch of points
pub fn miller_loop_batch_bls12_381(g1_points: &[[u64; 12]], g2_points: &[[u64; 24]]) -> [u64; 72] {
    // Before the loop starts, compute xp' = (-xp/yp)·1/(1+u) and yp' = (1/yp)·1/(1+u) for each point p
    let mut xp_primes: Vec<[u64; 12]> = Vec::with_capacity(g1_points.len());
    let mut yp_primes: Vec<[u64; 12]> = Vec::with_capacity(g1_points.len());
