...
///
/// The caller must ensure that `p` is a finite point on the Secp256k1 curve.
...
pub extern "C" fn syscall_secp256k1_dbl(
    ...
    #[cfg(zisk_guest)]
    ziskos_syscall!(zisk_definitions::SYSCALL_SECP256K1_DBL_ID, p);
    #[cfg(not(zisk_guest))]
    {
        let _p1 = [p.x, p.y].concat().try_into().unwrap();
        let mut p3: [u64; 8] = [0; 8];
        zisk_precomp_helpers::secp256k1_dbl(&_p1, &mut p3);
