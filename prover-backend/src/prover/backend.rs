...
impl ProverBackend {
    ...
    pub fn register_program(
        ...
        // No instruction table to register: the indexed Main packing is off while a Main row
        // carries lanes (see `zisk_pil::main_row`), so `packed` selects the full packed row.
