...
use executor::{initialize_executor_test, unit_test_hooks::UnitTestHookBag, ZiskExecutorTest};
...
impl UnitTestProver {
    ...
    /// CLI entry: verify constraints against the witness produced from a
    /// JSON file on disk. Used by `cargo-zisk unit-test --inputs FILE`.
    pub fn verify_constraints(
        ...
        if !inputs_json.exists() {
            anyhow::bail!("Inputs file not found at {}", inputs_json.display());
        }

        let start = std::time::Instant::now();
        self.executor.set_input_data_path(inputs_json);
        // No hooks on the CLI path.
        self.executor.set_hooks(UnitTestHookBag::new());
        self.run_verify(start, debug_info)
    }
    ...
    pub(crate) fn run_verify_input(
        ...
        // Always clear hooks for the next call.
        self.executor.set_hooks(UnitTestHookBag::new());
        result
    }
