...
impl AsmProver {
    ...
    fn start_asm_service(
        ...
        let mpi_broadcast_fn =
            (is_distributed && n_processes > 1 && with_hints && world_rank == 0).then(|| {
                let pctx = pctx.clone();
                ...
                Arc::new(move |data: &mut Vec<u8>| {
                    ...
                    tracing::info!(
                        "[MPI-TRACE] rank=0 SEND seq={seq} tid={tid:?} pre_size={in_size}"
                    );
                    pctx.mpi_ctx.broadcast(data);
                    let tag = data.first().copied().unwrap_or(255);
                    tracing::info!(
                        "[MPI-TRACE] rank=0 SEND seq={seq} tid={tid:?} done size={size} tag={tag}",
                        size = data.len(),
                    );
                    Ok(())
