...
pub trait DynUnitTestSm<F: PrimeField64>: Send + Sync {
    /// AIR id this SM serves. Object-safe forward of [`UnitTestSm::air_id`].
    fn air_id(&self) -> usize;
    /// SM name (JSON key / error label). Forward of [`UnitTestSm::name`].
    fn name(&self) -> &'static str;
    /// Trace rows per input. Forward of [`UnitTestSm::rows_per_input`].
    fn rows_per_input(&self) -> usize;
    /// Field-element width of one trace row (`<Row as TraceRow>::ROW_SIZE`),
    /// used by the dispatcher to slice the flat buffer into rows.
    ...
    /// Plan AIR instances for an erased input batch, downcasting `mgr` and
    /// `inputs` internally. Object-safe forward of [`UnitTestSm::plan`].
    fn plan_erased(
        &self,
    ...
    /// Compute the witness for one erased input chunk, downcasting `mgr` and
    /// `inputs` internally. Forward of [`UnitTestSm::compute_witness`].
