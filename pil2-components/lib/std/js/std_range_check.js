...
module.exports = class RangeCheck extends WitnessCalculatorComponent {
    ...
    setup(airInstance) {
        ...
        const hints_rc = hints.filter(h => h.name === 'range_def');
    ...
    assignValues(values, min, max) {
        if (min < 0n && max < 0n) {
            min += this.proofCtx.F.p;
            max += this.proofCtx.F.p;
        }

        const range = this.ranges.find(range => range.min === min && range.max === max);
        if (!range) {
            console.log(this.ranges);
            throw new Error(`Range not found: [min,max] = [${min},${max}]`);
        }
