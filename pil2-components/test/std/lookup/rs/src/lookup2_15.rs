    ...
    Standard: Distribution<F>,
{
    ...
    pub fn execute(&self, pctx: Arc<ProofCtx<F>>, ectx: Arc<ExecutionCtx>, sctx: Arc<SetupCtx>) {
        ...
        let air_instance = AirInstance::new(sctx.clone(), LOOKUP_AIRGROUP_ID, LOOKUP_2_15_AIR_IDS[0], None, buffer);

        let (is_myne, gid) =
            ectx.dctx.write().unwrap().add_instance(LOOKUP_AIRGROUP_ID,  LOOKUP_2_15_AIR_IDS[0], 1);
        if is_myne {
