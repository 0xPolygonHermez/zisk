...
impl StdSum {
    ...
    fn calculate_witness<F>(
        ...
        for hint in im_hints {
            let im = get_hint_field::<F>(setup, hint, "reference", true, false);
            let num = get_hint_field::<F>(setup, hint, "numerator", false, false);
            let den = get_hint_field::<F>(setup, hint, "denominator", false,  false);
        ...
        let mut gsum = get_hint_field::<F>(setup, gsum_hint, "reference", true, false); // column to feed
        let expr = get_hint_field::<F>(setup, gsum_hint, "expression", false, false);
