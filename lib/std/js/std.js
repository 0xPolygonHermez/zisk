...
module.exports = class STD extends WitnessCalculatorComponent {
    ...
    setupRange(airInstance, min, max) {
        const RC = this.components['RangeCheck'];
        return RC.setup(airInstance, min, max);
    }

    rangeCheck(range, values) {
        const RC = this.components['RangeCheck'];
        RC.assignValues(range, values);
    }
        ...
        if (stageId === 2) {
            ...
            for (const rcair of rangeCheckAirs) {
                if (rcair.subproofId == subproofId) {
                    ...
                    await this.decider(rcair.stageId, rcair.subproofId, 0, 0, publics);
                    return;
                }
            }
