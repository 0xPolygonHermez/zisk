...
module.exports = class STD extends WitnessCalculatorComponent {
    ...
    redirectValues(airInstance, values) {
        ...
        const hint_rc = hints.find(h => h.name === 'range_check');

        const predefined = getHintField(airInstance.ctx, hint_rc, "predefined");
        const min = getHintField(airInstance.ctx, hint_rc, "min");
        const max = getHintField(airInstance.ctx, hint_rc, "max");

        if (predefined && min > 0 && max <= TWOBYTES) {
            if (min == 0 && (max == BYTE || max == TWOBYTES)) {
                RC.proves(values);
            } else if (max <= TWOBYTES) {
                RC.proves(values-min);
                RC.proves(max-values);
            } else {
    ...
    async witnessComputation(stageId, subproofId, airInstance, publics) {
        ...
        const U8 = this.proofCtx.airout.subproofs.find(s => s.name === 'U8Air');
        const U16 = this.proofCtx.airout.subproofs.find(s => s.name === 'U16Air');
        const Specific = this.proofCtx.airout.subproofs.find(s => s.name === 'SpecifiedRanges');

        if (stageId === 1) {
            if (U8 && U8.subproofId == subproofId) {
                    ...
                    for (let i = 0; i < mailbox.length; i++) {
                        ...
                        if ((msg.type && msg.payload.data) && (msg.type === 'notification' && msg.payload.data === 'finished')) {
                            ...
                            if (rcHints.length > 0) {
                                ...
                                // I should not terminate here, but wait until the last subproof is processsed
                                this.terminate = true;
                            }
                        }
                    }
        ...
        if (stageId === 2) {
            ...
            if (U8 && U8.subproofId == subproofId) {
                ...
                await this.decider(U8.stageId, U8.subproofId, 0, 0, publics);
                // await this.components['RangeCheck'].witnessComputation(stageId, subproofId, airInstance, publics);
                return;
            }
