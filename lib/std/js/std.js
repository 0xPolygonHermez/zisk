...
module.exports = class Std extends WitnessCalculatorComponent {
    ...
    // This function should decide what to call in the std depending on what the pilout needs
    async witnessComputation(stageId, subproofId, airId, instanceId, publics) {
        console.log(`### STD stage:${stageId} subproof:${subproofId} air:${airId} instance:${instanceId}`);

        const instanceData = await this.wcManager.receiveData(this, "std");
        for (let i = 0; i < instanceData.length; i++) {
            const info = instanceData[i];
            if (info.type === 'notification' && info.payload.data === 'finished') {
                stageId = info.payload.stageId;
                subproofId = info.payload.subproofId;
        ...
        if (gprod_hint && stageId === 2) {
            ...
            await prod.witnessComputation(stageId, subproofId, airId, instanceId, publics);
        }
