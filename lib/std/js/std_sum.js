...
module.exports = class Sum extends WitnessCalculatorComponent {
    ...
    async witnessComputation(stageId, subproofId, airInstance, publics) {
        ...
        if (stageId === 1) {
            ...
            const assumesHint = this.hints.find(h => h.name === 'sum_assumes');
            const provesHint = this.hints.find(h => h.name === 'sum_proves');
            const gsumHint = this.hints.find(h => h.name === 'gsum_col');

            // In this stage we compute the multiplicity column
            const gsumIdx = getHintField(airInstance.ctx, gsumHint, "reference");
            const numIdx = getHintField(airInstance.ctx, gsumHint, "numerator");
            const denIdx = getHintField(airInstance.ctx, gsumHint, "denominator");
            const resultIdx = getHintField(airInstance.ctx, gsumHint, "result");
            ...
            for (let h of this.hints) {
                if (h.name === 'sum_proves') {
                    ...
                    // Send the assumes
                    await this.sendData(this, "Basic Rom", {"airId": 0});
                }
            }
            EXIT;
            ...
            // In this stage we compute the grand-sum column
