...
module.exports = class Prod extends WitnessCalculatorComponent {
    ...
    async witnessComputation(stageId, subproofId, airInstance, publics) {
        ...
        // Here we compute the grand-product column
        const gprodIdx = getHintField(airInstance.ctx, this.hint, "reference", true).id;
        ...
        const numerator = getHintField(airInstance.ctx, this.hint, "numerator", false, true);
        ...
        gprod[0] = F.div(Array.isArray(numerator) ? numerator[0] : numerator, Array.isArray(denominator) ? denominator[0] : denominator);
