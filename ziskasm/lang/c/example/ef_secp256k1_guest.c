...
static uint8_t g_out[32];                       /* .bss (address taken -> allocated) */
static volatile uint32_t g_dataval = 0x11223344; /* .data (volatile -> not folded) */
int main(void) {
    bool ok = false;
    zkvm_status st = zkvm_secp256k1_verify(&MSG, &SIG, &PUB, &ok);
    g_out[0] = (st == ZKVM_EOK) ? 1 : 0;
    g_out[1] = ok ? 1 : 0;
    g_out[2] = (uint8_t)g_dataval;
    emit32(g_out);
    return 0;
}
