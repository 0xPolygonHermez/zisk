...
- **Entry point**: the `_start` label (the BIOS jumps there); no fixed boot addr.
- **Instruction stride is fixed at 4 bytes** (the assembler owns addresses).
- **ret-to-BIOS exit** (the intended final model): the BIOS enters `_start` with
  the address of its output-finalization code in `r1` (a `call`). `_start` saves
  `r1` (it is clobbered by its own `call _zisk_main`), restores it, and `ret`s;
  the BIOS then reads `OUTPUT_ADDR` and ends. No `end`/syscall exit in the program.
