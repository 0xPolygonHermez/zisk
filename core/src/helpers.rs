...
use zisk_precomp_helpers::{blake2b_round, blake2s_f, blake3_f};
...
/// Blake2s permutation (the 10 rounds of G-mixing, no feed-forward) over the 16-u32 state,
/// viewing each u64 word as two little-endian u32 words.
pub fn blake2sf(state: &mut [u64; 8], input: &[u64; 8]) {
    let state_u32: &mut [u32; 16] = unsafe { &mut *(state.as_mut_ptr() as *mut [u32; 16]) };
    let input_u32: &[u32; 16] = unsafe { &*(input.as_ptr() as *const [u32; 16]) };
