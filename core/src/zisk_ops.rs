...
// `OpType` enum + its `ZiskOperationType`/`Display`/`FromStr` mappings live in
// `zisk_op_type_table.rs`, pulled in at item position (Phase 0.2a, no behavior change).
include!("zisk_op_type_table.rs");
...
/// type.
// The `define_ops!` invocation (the opcode table) lives in `zisk_ops_table.rs` and is
// pulled in here at item position, so the macro + referenced fns/consts/OpType variants
// resolve in this module's scope. Phase 0.2a: extracted verbatim, no behavior change.
include!("zisk_ops_table.rs");
