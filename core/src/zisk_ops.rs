...
/// type.
// The opcode table = hand-written vanilla-ISA rows (`zisk_ops_table.rs`) wrapped by the
// generated `zisk_ops_with_precompiles!` macro (`precompile_ops.gen.rs`), which appends
// the enabled precompiles' rows. Both are pulled in at item position so the `define_ops!`
// macro + referenced `opc_*`/cost/`OpType` names resolve in this module's scope. The
// macro def must precede the invocation.
include!("precompile_ops.gen.rs");
