...
mod shmem_utils;
mod shmem_writer;
// Off Linux-x86_64 the ASM runners and shmem transport above are `cfg`-gated
// out; this single module supplies the cross-platform surface that still has to
// compile (see its own docs). Replaces the former per-module `*_stub` files.
...
// Off Linux-x86_64: the cross-platform stub surface (`AsmRunnerMO`,
// `AsmRunnerRH`, `HintsShmem`, `InputsShmemWriter`).
#[cfg(not(all(target_os = "linux", target_arch = "x86_64")))]
