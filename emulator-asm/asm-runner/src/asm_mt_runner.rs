...
impl AsmRunnerMT {
    ...
    pub fn run_and_count<T: Task>(
        ...
        let shmem_input_name = format!("ZISK_{}_MT_input", world_rank);
        let shmem_output_name = format!("ZISK_{}_MT_output", world_rank);
        let sem_chunk_done_name = format!("/ZISK_{}_MT_chunk_done", world_rank);
