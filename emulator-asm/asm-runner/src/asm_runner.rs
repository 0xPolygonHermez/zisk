use crate::asm_output::{OutputChunk, OutputHeaderC};
...
impl AsmRunner {
    ...
    fn map_output<'a>(shmem_output_name: String) -> (*mut c_void, Vec<OutputChunk<'a>>) {
        ...
        // Read Output Header
        let output_header_size = size_of::<OutputHeaderC>();
        let mapped_ptr =
        ...
        let output_header = OutputHeaderC::map_output_header(mapped_ptr);
        ...
        unsafe {
            ...
            for _ in 0..num_chunks {
                let data = OutputHeaderC::map_output2(&mut mapped_ptr);
                // println!("Chunk: {:?}", data);
