    ...
    Standard: Distribution<F>,
{
    ...
    pub fn new(wcm: Arc<WitnessManager<F>>, std_lib: Arc<Std<F>>) -> Arc<Self> {
        ...
        wcm.register_component(
            range_check1.clone(),
            Some(RANGE_CHECK_3_AIRGROUP_ID),
            Some(RANGE_CHECK_3_AIR_IDS),
        );
    ...
    Standard: Distribution<F>,
{
    fn calculate_witness(
        ...
        if stage == 1 {
            ...
            let num_rows = pctx
                .pilout
                .get_air(RANGE_CHECK_3_AIRGROUP_ID, RANGE_CHECK_3_AIR_IDS[0])
                .num_rows();
            let mut trace =
                RangeCheck30Trace::map_buffer(buffer.as_mut_slice(), num_rows, offsets[0] as usize)
                    .unwrap();
            ...
            for i in 0..num_rows {
                ...
                self.std_lib
                    .range_check(trace[i].c1, range1.0.clone(), range1.1.clone());
                self.std_lib
                    .range_check(trace[i].c2, range2.0.clone(), range2.1.clone());
            }
