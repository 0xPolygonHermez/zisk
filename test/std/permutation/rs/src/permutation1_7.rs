...
use crate::{Permutation1_71Trace, PERMUTATION_1_7_AIR_IDS, PERMUTATION_AIRGROUP_ID};
...
{
    const MY_NAME: &'static str = "Permutation1_7";

    pub fn new(wcm: Arc<WitnessManager<F>>) -> Arc<Self> {
        let permutation1_7 = Arc::new(Self {
            _phantom: std::marker::PhantomData,
        });

        wcm.register_component(
            permutation1_7.clone(),
            Some(PERMUTATION_AIRGROUP_ID),
            Some(PERMUTATION_1_7_AIR_IDS),
        );
    ...
    pub fn execute(&self, pctx: Arc<ProofCtx<F>>, ectx: Arc<ExecutionCtx>, sctx: Arc<SetupCtx>) {
        ...
        let (buffer_size, _) = ectx
            .buffer_allocator
            .as_ref()
            .get_buffer_info(&sctx, PERMUTATION_AIRGROUP_ID, PERMUTATION_1_7_AIR_IDS[0])
            .unwrap();
    ...
    Standard: Distribution<F>,
{
    fn calculate_witness(
        ...
        if stage == 1 {
            ...
            let mut trace = Permutation1_71Trace::map_buffer(
                buffer.as_mut_slice(),
                num_rows,
                offsets[0] as usize,
            )
