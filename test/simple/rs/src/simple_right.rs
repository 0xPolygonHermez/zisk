    ...
    Standard: Distribution<F>,
{
    ...
    pub fn execute(&self, pctx: &mut ProofCtx<F>, ectx: &ExecutionCtx, _sctx: &SetupCtx) {
        let (buffer_size, _) = ectx
            .buffer_allocator
            .as_ref()
            .get_buffer_info("Simple".into(), SIMPLE_RIGHT_AIR_IDS[0])
            .unwrap();

        let buffer = vec![F::zero(); buffer_size as usize];

        let air_instance =
            AirInstance::new(SIMPLE_AIRGROUP_ID, SIMPLE_RIGHT_AIR_IDS[0], None, buffer);
    ...
    fn calculate_witness(
        ...
        if stage == 1 {
            let (_, offsets) = ectx
                .buffer_allocator
                .as_ref()
                .get_buffer_info("Simple".into(), air_id)
                .unwrap();
            ...
            let mut trace =
                SimpleRight1Trace::map_buffer(buffer.as_mut_slice(), num_rows, offsets[0] as usize)
                    .unwrap();
