...
pub fn trace_setup_handler(pilout: &PilOutProxy, subproof_id: usize) -> Result<String, Box<dyn std::error::Error>> {
    ...
    let subproof = &pilout.subproofs[subproof_id];
    let subproof_name = subproof.name.as_ref().unwrap().clone() + ".";

    for (air_index, air) in subproof.airs.iter().enumerate() {
        let mut trace = format!("trace!({} {{\n", air.name.as_ref().unwrap());

        for witness_col in &witness_cols {
            if witness_col.subproof_id.unwrap() == subproof_id as u32 && witness_col.air_id.unwrap() == air_index as u32
            {
