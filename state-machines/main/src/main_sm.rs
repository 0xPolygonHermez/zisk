...
impl MainSM {
    ...
    pub fn execute<F: AbstractField + Send + Sync>(
        ...
        pool.scope(|scope| {
            ...
            let callback = |inputs: EmuTrace| self.emulator_callback(inputs, scope, pctx, ectx);
    ...
    fn emulator_callback<'a, F: AbstractField + Send + Sync>(
        ...
        scope.spawn(move |scope| {
            if let Ok(mut inputs) = self_inputs.lock() {
                inputs.steps.extend(emu_inputs.steps);

                while inputs.steps.len() >= Self::MAX_ACCUMULATED {
                    let inputs_batch =
                        inputs.steps.drain(..Self::MAX_ACCUMULATED).collect::<Vec<_>>();
    ...
    /// * `pctx` - Proof context to interact with the proof system
    /// * `ectx` - Execution context to interact with the execution environment
    fn prove<F: AbstractField>(inputs: EmuTrace, pctx: &ProofCtx<F>, ectx: &ExecutionCtx) {
