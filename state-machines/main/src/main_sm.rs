...
pub struct MainSM {
    ...
    callback_inputs: Arc<Mutex<EmuTrace>>,
    //State machines
...
impl MainSM {
    ...
    /// processed
    const MAX_ACCUMULATED: usize = 2usize.pow(16);
    ...
    pub fn new<F>(
        ...
        let main = Arc::new(Self {
            ...
            mem_sm,
            arith_sm,
            callback_inputs: Arc::new(Mutex::new(EmuTrace::default())),
            main_pk,
        });
    ...
    pub fn execute<F: AbstractField + Send + Sync>(
        ...
        pool.scope(|scope| {
            ...
            let callback = |inputs: EmuTrace| {
                Self::emulator_callback(&self.zisk_rom, inputs, scope, pctx, ectx)
            };
    ...
    fn emulator_callback<'a, F: AbstractField + Send + Sync>(
        ...
        scope.spawn(move |scope| {
            Self::prove(zisk_rom, emu_inputs, pctx, ectx);
        });
    }
    ...
    /// * `ectx` - Execution context to interact with the execution environment
    #[inline(always)]
    fn prove<F: AbstractField>(
        zisk_rom: &ZiskRom,
        inputs: EmuTrace,
        pctx: &ProofCtx<F>,
        ectx: &ExecutionCtx,
    ) {
        info!("{}: ··· prove_inputs_batch() with {} inputs", Self::MY_NAME, inputs.steps.len());

        let processed_slice = match ZiskEmulator::process_slice::<F>(zisk_rom, &inputs) {
            Ok(slice) => slice,
            Err(e) => panic!("Error processing slice: {:?}", e),
        };

        // // Capture inputs.len() and calculate the next power of two for the number of rows needed
        // let num_rows = inputs.steps.len().next_power_of_two();

        // // Use BufferAllocator to get the buffer size and create a new trace segment for the main
        // let size =
        // ectx.buffer_allocator.as_ref().get_buffer_info(Path::new("../pil2-components/test/
        // fibonacci/build_x/provingKey/build/FibonacciSquare/airs/FibonacciSquare_0/air/"));
        // if let Err(e) = size {
        ...
        // for (idx, input) in inputs.steps.iter().enumerate() {
