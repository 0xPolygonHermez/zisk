...
use proofman_common::{AirInstance, AirInstanceCtx, BufferAllocator, ExecutionCtx, ProofCtx};
...
impl MainSM {
    ...
    pub fn new<F>(
        ...
        // Check if the maximum number of accumulated inputs is set to 0
        let main = Arc::new(Self {
            elf_rom: elf_rom.to_path_buf(),
            mem_sm,
            arith_sm,
    ...
    pub fn execute<F: AbstractField + Send + Sync>(
        ...
        let emulator_options = EmuOptions {
            elf: Some(self.elf_rom.clone().display().to_string()),
