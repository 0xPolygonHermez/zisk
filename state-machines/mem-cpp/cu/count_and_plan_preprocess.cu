...
//
// Public API + types live in count_and_plan.cuh. This file owns:
//   * the preprocessing kernels (decode_count, decode_emit, blockop_emit,
//     extract_sorted_*, gather_ram_events_with_hist, state_machine_by_run_with_hist,
//     compact_kernel_with_shift, add_const)
//   * the PairSortGPU kernels (instance_boundaries, chunk_fml_count_gappy,
//     build_metas, compute_addr_offsets)
//   * the full struct definitions for PotentialEmit / BlockOpSpill /
//     ChunkCounters (forward-declared in the header)
//   * the CountAndPlan member-function bodies
//   * the binary-meta save helpers (save_metas_begin/append/end)
// =====================================================================
...
CountAndPlan::CountAndPlan()
    : h_active_first_(MAX_ACTIVE),
      h_active_last_(MAX_ACTIVE),
      metas_(MAX_ACTIVE)
{}
...
bool CountAndPlan::setup(void* d_buf, size_t bytes,
    ...
    if (max_chunks == 0 || max_memops_per_chunk == 0) {
        fprintf(stderr,
                "CountAndPlan::setup ERROR: max_chunks=%u, max_memops_per_chunk=%u — both must be > 0\n",
                max_chunks, max_memops_per_chunk);
        return false;
    }

    max_chunks_           = max_chunks;
    max_memops_per_chunk_ = max_memops_per_chunk;
    max_pot_per_chunk_    = max_memops_per_chunk * POTENTIAL_FACTOR;
    ...
    auto fixed_bytes = [&]() -> size_t {
        ...
        take((size_t)MAX_ACTIVE * 4);
        take((size_t)MAX_ACTIVE * 4);
        take((size_t)MAX_ACTIVE * 4);
        take((size_t)MAX_ACTIVE * MAX_CHUNKS * 3 * 4);
        take((size_t)MAX_ACTIVE * MAX_CHUNKS * 4);
        take((size_t)MAX_ACTIVE * 4 * 4);
        take((size_t)MAX_ACTIVE * (INSTANCE_SIZE_MAX + 1) * 4);
        take((size_t)MAX_ACTIVE * 4);
        take(3 * 4);
        take(4);
        take(sizeof(ChunkCounters));
        take((size_t)max_chunks_ * 4);
        take((size_t)max_chunks_ * 4);
        take(((size_t)max_chunks_ + 1) * 4);
        for (int s = 0; s < N_PRE_STREAMS; s++) {
            take((size_t)max_pot_per_chunk_    * sizeof(PotentialEmit));
            take((size_t)max_pot_per_chunk_    * 4);
            take(((size_t)max_pot_per_chunk_ + 1) * 4);
            take((size_t)max_memops_per_chunk_ * sizeof(MemOp));
            take(((size_t)max_memops_per_chunk_ + 1) * 4);
            take(((size_t)max_memops_per_chunk_ + 1) * 4);
            take((size_t)MAX_BLOCKOP_SPILL_PER_CHUNK * sizeof(BlockOpSpill));
            take(4);
            take((size_t)max_memops_per_chunk_);
            take((size_t)max_pot_per_chunk_ * 8);
            take((size_t)max_pot_per_chunk_ * 8);
            take((size_t)max_pot_per_chunk_ * 4);
            take((size_t)max_pot_per_chunk_ * 4);
            take((size_t)max_pot_per_chunk_ * 4);
    ...
    if (d_buf != nullptr && bytes > 0) {
        if (bytes < fixed_bytes) {
            fprintf(stderr,
                    ...
                    bytes, fixed_bytes, max_chunks_, max_memops_per_chunk_);
            return false;
        }
    ...
    d_active_ids_             = (uint32_t*)take((size_t)MAX_ACTIVE * 4);
    d_active_first_           = (uint32_t*)take((size_t)MAX_ACTIVE * 4);
    d_active_last_            = (uint32_t*)take((size_t)MAX_ACTIVE * 4);
    d_fml_                    = (uint32_t*)take((size_t)MAX_ACTIVE * MAX_CHUNKS * 3 * 4);
    d_result_nops_            = (uint32_t*)take((size_t)MAX_ACTIVE * MAX_CHUNKS * 4);
    d_meta_scalars_           = (uint32_t*)take((size_t)MAX_ACTIVE * 4 * 4);
    d_addr_offsets_           = (uint32_t*)take((size_t)MAX_ACTIVE * (INSTANCE_SIZE_MAX + 1) * 4);
    d_offset_starts_          = (uint32_t*)take((size_t)MAX_ACTIVE * 4);
    ...
    d_gappy_offsets_          = (uint32_t*)take((size_t)max_chunks_ * 4);
    d_chunk_lens_             = (uint32_t*)take((size_t)max_chunks_ * 4);
    d_packed_chunk_offsets_   = (uint32_t*)take(((size_t)max_chunks_ + 1) * 4);

    for (int s = 0; s < N_PRE_STREAMS; s++) {
        d_potentials_[s]        = (PotentialEmit*)take((size_t)max_pot_per_chunk_    * sizeof(PotentialEmit));
        d_emit_bits_[s]         = (uint32_t*)     take((size_t)max_pot_per_chunk_    * 4);
        d_final_offsets_[s]     = (uint32_t*)     take(((size_t)max_pot_per_chunk_ + 1) * 4);
        d_memops_[s]            = (MemOp*)        take((size_t)max_memops_per_chunk_ * sizeof(MemOp));
        d_counts_[s]            = (uint32_t*)     take(((size_t)max_memops_per_chunk_ + 1) * 4);
        d_potential_offsets_[s] = (uint32_t*)     take(((size_t)max_memops_per_chunk_ + 1) * 4);
        d_spill_[s]             = (BlockOpSpill*) take((size_t)MAX_BLOCKOP_SPILL_PER_CHUNK * sizeof(BlockOpSpill));
        d_spill_count_[s]       = (uint32_t*)     take(4);
        d_spill_status_[s]      = (uint8_t*)      take((size_t)max_memops_per_chunk_);
        d_ram_keys_[s]          = (uint64_t*)     take((size_t)max_pot_per_chunk_ * 8);
        d_ram_keys_sorted_[s]   = (uint64_t*)     take((size_t)max_pot_per_chunk_ * 8);
        d_sorted_addr_[s]       = (uint32_t*)     take((size_t)max_pot_per_chunk_ * 4);
        d_ram_vals_sorted_[s]   = (uint32_t*)     take((size_t)max_pot_per_chunk_ * 4);
        d_run_lengths_[s]       = (uint32_t*)     take((size_t)max_pot_per_chunk_ * 4);
        d_run_offsets_[s]       = (uint32_t*)     take(((size_t)max_pot_per_chunk_ + 1) * 4);
    ...
    CUDA_CHECK(cudaMallocHost(&h_memops_,
        (size_t)max_chunks_ * max_memops_per_chunk_ * sizeof(MemOp)));
    CUDA_CHECK(cudaMallocHost(&h_n_emits_all_, (size_t)max_chunks_ * 4));
    CUDA_CHECK(cudaMallocHost(&h_result_nops_, (size_t)MAX_ACTIVE * MAX_CHUNKS * 4));
    CUDA_CHECK(cudaMallocHost(&h_meta_scalars_, (size_t)MAX_ACTIVE * 4 * 4));
...
size_t CountAndPlan::run(uint32_t worker_id, void* metas_out, size_t metas_out_bytes) {
    if (n_chunks_ == 0) {
    ...
    return serialize_metas_(metas_out, metas_out_bytes);
}

void CountAndPlan::saveMetas(const std::string& path) {
    FILE* f = save_metas_begin(path);
    for (uint32_t i = 0; i < num_active_; i++)
        save_metas_append(f, metas_[i]);
    save_metas_end(f, num_active_);
}
...
void CountAndPlan::reset() {
    ...
    metas_.assign(MAX_ACTIVE, InstanceMeta{});
...
void CountAndPlan::prepare_global_() {
    ...
    for (uint8_t r = 0; r < 3; r++) {
        ...
        if (num_inst_[r] > MAX_INST[r]) {
            std::cerr << "CountAndPlan: too many instances in " << REGION_NAME[r]
                      << " (" << num_inst_[r] << " > " << MAX_INST[r] << ")" << std::endl;
            std::exit(1);
        }
...
void CountAndPlan::set_active_worker_(uint32_t w) {
...
void CountAndPlan::process_worker_(uint32_t worker_id) {
    set_active_worker_(worker_id);
    ...
    for (uint8_t r = 0; r < 3; r++) {
        for (uint32_t j = 0; j < num_active_per_[r]; j++, ai++) {
            ...
            uint32_t num_addrs = h_active_last_[ai] - h_active_first_[ai] + 1;
            metas_[ai].count_per_chunk = {h_result_nops_ + (size_t)ai * n_chunks_, n_chunks_};
            metas_[ai].addr_offsets    = {h_offsets_buf_ + h_offset_starts[ai], num_addrs};
        }
    }
}

size_t CountAndPlan::serialize_metas_(void* out, size_t out_bytes) {
    size_t needed = 4;
    for (uint32_t i = 0; i < num_active_; i++) {
        needed += 4 * 10
                + metas_[i].count_per_chunk.size() * 4
                + metas_[i].addr_offsets.size()    * 4;
    }
    ...
    if (out_bytes < needed) {
        fprintf(stderr,
                "CountAndPlan::run ERROR: metas_out is %zu bytes, need %zu\n",
                out_bytes, needed);
        return 0;
    }
    uint8_t* p = (uint8_t*)out;
    auto wr = [&](const void* src, size_t b) { std::memcpy(p, src, b); p += b; };
    uint32_t n_metas = num_active_;
    wr(&n_metas, 4);
    for (uint32_t i = 0; i < num_active_; i++) {
        const InstanceMeta& m = metas_[i];
        uint8_t  type = m.type;
        uint8_t  pad[3] = {0, 0, 0};
        uint32_t cps  = (uint32_t)m.count_per_chunk.size();
        uint32_t aos  = (uint32_t)m.addr_offsets.size();
        ...
        wr(&m.first_addr, 4);
        wr(&m.last_addr, 4);
        wr(&m.first_addr_chunk, 4);
        wr(&m.first_addr_skip, 4);
        wr(&m.last_addr_chunk, 4);
        wr(&m.last_addr_include, 4);
        wr(&cps, 4);
        wr(&aos, 4);
        wr(m.count_per_chunk.data(), cps * 4);
        wr(m.addr_offsets.data(),    aos * 4);
    }
    return needed;
}
