...
static void generate_mem_segments_into(MemSegments dest[MEM_TYPES], const std::vector<InstanceMeta> &instances) {
    ...
    for (const auto &instance : instances) {
        ...
        // Sparse offsets — two parallel u32 vectors. Total bytes per inject is
        // bounded by 8 * offset_changes_count (typically <30 MB on the
        // reference input, vs. 341 MB for the old dense addr_offsets).
        segment->offset_change_slots.assign(
            instance.offset_change_slots,
            instance.offset_change_slots + instance.offset_changes_count);
        segment->offset_change_values.assign(
            instance.offset_change_values,
            instance.offset_change_values + instance.offset_changes_count);
                                               ...
                                               const uint32_t *&values_out, uint32_t &count)
{
    ...
    if (segment) {
        ...
        range_slots_out       = segment->addr_range_slots;
        count                 = segment->offset_change_slots.size();
        values_out            = segment->offset_change_values.data();
        return segment->offset_change_slots.data();
    } else {
