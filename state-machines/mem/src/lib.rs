...
use mem_common::{MemModuleSegmentCheckPoint, RAM_W_ADDR_INIT};
...
unit_test_sm! {
    RomDataSm => {
        ...
        compute: |sm, _sctx, inputs, buf, packed| {
            let prev = MemPreviousSegment { addr: ROM_DATA_W_ADDR_INIT, step: 0, value: 0 };
            sm.compute_witness(
                &inputs,
                SegmentId(0),
                true,
...
unit_test_sm! {
    InputDataSm => {
        ...
        compute: |sm, _sctx, inputs, buf, packed| {
            let prev = MemPreviousSegment { addr: INPUT_DATA_W_ADDR_INIT, step: 0, value: 0 };
            sm.compute_witness(
                &inputs,
                SegmentId(0),
                true,
