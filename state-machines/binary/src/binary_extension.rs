...
impl<F: PrimeField64> BinaryExtensionSM<F> {
    ...
    pub fn compute_witness(&self, inputs: &[Vec<BinaryInput>], trace_buffer: Vec<F>) -> AirInstance<F> {
        ...
        binary_e_trace.row_slice_mut()[total_inputs..num_rows].fill(padding_row);
