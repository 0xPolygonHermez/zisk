#![allow(dead_code)]
#![allow(clippy::manual_range_contains, clippy::nonminimal_bool)]
// @generated by frops-analyzer — do not edit by hand.
// generated with: max-table=25165824 partition-bits=21 low-cap=4096 max-regions-per-op=4 table-cost=3 nodes=1
use std::error::Error;
use zisk_core::zisk_ops::ZiskOp;
use zisk_sm_frequent_ops::FrequentOpsHelpers;

const OP_TABLE_OFFSETS_START: usize = 33;
const OP_TABLE_OFFSETS: [usize; 34] = [
    0, 2641408, 3080704, 0, 3405312, 3426368, 3427168, 0, 3427424, 0, 0, 0, 0, 0, 0, 3427936,
    3432032, 3433582, 3499118, 0, 0, 3499123, 3500698, 3500718, 0, 3504814, 0, 0, 3504943, 0,
    3504975, 3505039, 3514839, 3518935,
];

#[derive(Debug, Clone)]
pub struct BinaryExtensionFrops {
    table: FrequentOpsHelpers,
}

const FREQUENT_OP_EMPTY: usize = 256;

impl Default for BinaryExtensionFrops {
    fn default() -> Self {
        Self::new()
    }
}

impl BinaryExtensionFrops {
    pub const TABLE_ID: usize = 5012;
    pub const NO_FROPS: usize = FrequentOpsHelpers::NO_FROPS;
    pub fn new() -> Self {
        Self { table: FrequentOpsHelpers::new() }
    }

    pub fn build_table(&mut self) {
        // op sll
        {
            let mut ops: Vec<[u64; 2]> = Vec::new();
            // low_rect: a < 4096 && b < 161
            for a in 0..4096 {
                for b in 0..161 {
                    ops.push([a, b]);
                }
            }
            // mid_box: a >= 4096 && a < 0x10000 && (a & 7) == 0 && b < 243
            for a in (0x1000..0x10000).step_by(8) {
                for b in 0..243 {
                    ops.push([a, b]);
                }
            }
            // mid_box: a >= 0xFFFFF007 && a < 0x100000007 && (a & 7) == 7 && b < 225
            for a in (0xFFFFF007..0x100000007).step_by(8) {
                for b in 0..225 {
                    ops.push([a, b]);
                }
            }
            // mid_box: a >= 0xFFFFFFFFFEFEF007 && a < 0xFFFFFFFFFEFF0007 && (a & 7) == 7 && b == 32
            for a in (0xFFFFFFFFFEFEF007..0xFFFFFFFFFEFF0007).step_by(8) {
                for b in 0x20..0x21 {
                    ops.push([a, b]);
                }
            }
            self.table.add_ops(ZiskOp::SLL, &mut ops, true);
        }
        // op srl
        {
            let mut ops: Vec<[u64; 2]> = Vec::new();
            // low_rect: a < 4096 && b < 65
            for a in 0..4096 {
                for b in 0..65 {
                    ops.push([a, b]);
                }
            }
            // mid_box: a >= 0xFFFFFFFFFFF000 && a < 0x100000000001000 && (a & 7) == 0 && b >= 1 && b < 121
            for a in (0xFFFFFFFFFFF000..0x100000000001000).step_by(8) {
                for b in 0x1..0x79 {
                    ops.push([a, b]);
                }
            }
            // mid_box: a >= 0x1102000000000000 && a < 0x1102000000001000 && (a & 7) == 0 && b >= 8 && b < 57
            for a in (0x1102000000000000..0x1102000000001000).step_by(8) {
                for b in 0x8..0x39 {
                    ops.push([a, b]);
                }
            }
            // mid_box: a >= 0xFFFFFFFF00000000 && a < 0xFFFFFFFF00001000 && (a & 7) == 0 && b >= 8 && b < 57
            for a in (0xFFFFFFFF00000000..0xFFFFFFFF00001000).step_by(8) {
                for b in 0x8..0x39 {
                    ops.push([a, b]);
                }
            }
            self.table.add_ops(ZiskOp::SRL, &mut ops, true);
        }
        // op sra
        {
            let mut ops: Vec<[u64; 2]> = Vec::new();
            // low_rect: a < 4096 && b < 64
            for a in 0..4096 {
                for b in 0..64 {
                    ops.push([a, b]);
                }
            }
            // mid_box: a >= 4096 && a < 24576 && b >= 3 && b < 6
            for a in 0x1000..0x6000 {
                for b in 0x3..0x6 {
                    ops.push([a, b]);
                }
            }
            // mid_box: a >= 0x6000000000000000 && a < 0x6000000000001000 && (a & 7) == 0 && b == 56
            for a in (0x6000000000000000..0x6000000000001000).step_by(8) {
                for b in 0x38..0x39 {
                    ops.push([a, b]);
                }
            }
            // mid_box: a >= 0x6100000000000000 && a < 0x6100000000001000 && (a & 7) == 0 && b == 56
            for a in (0x6100000000000000..0x6100000000001000).step_by(8) {
                for b in 0x38..0x39 {
                    ops.push([a, b]);
                }
            }
            self.table.add_ops(ZiskOp::SRA, &mut ops, true);
        }
        // op srl_w
        {
            let mut ops: Vec<[u64; 2]> = Vec::new();
            // low_rect: a < 530 && b < 32
            for a in 0..530 {
                for b in 0..32 {
                    ops.push([a, b]);
                }
            }
            // mid_box: a >= 0xFFFFA007 && a < 0x100000007 && (a & 7) == 7 && b == 24
            for a in (0xFFFFA007..0x100000007).step_by(8) {
                for b in 0x18..0x19 {
                    ops.push([a, b]);
                }
            }
            // mid_box: a >= 0x11020000000000 && a < 0x11020000001000 && (a & 7) == 0 && b == 24
            for a in (0x11020000000000..0x11020000001000).step_by(8) {
                for b in 0x18..0x19 {
                    ops.push([a, b]);
                }
            }
            // mid_box: a >= 0x1102000000000000 && a < 0x1102000000001000 && (a & 7) == 0 && b == 24
            for a in (0x1102000000000000..0x1102000000001000).step_by(8) {
                for b in 0x18..0x19 {
                    ops.push([a, b]);
                }
            }
            self.table.add_ops(ZiskOp::SRL_W, &mut ops, true);
        }
        // op sra_w
        {
            let mut ops: Vec<[u64; 2]> = Vec::new();
            // low_rect: a < 25 && b < 32
            for a in 0..25 {
                for b in 0..32 {
                    ops.push([a, b]);
                }
            }
            self.table.add_ops(ZiskOp::SRA_W, &mut ops, true);
        }
        // op signextend_b
        {
            let mut ops: Vec<[u64; 2]> = Vec::new();
            // low_rect: a == 0 && b < 256
            for a in 0..1 {
                for b in 0..256 {
                    ops.push([a, b]);
                }
            }
            self.table.add_ops(ZiskOp::SIGNEXTEND_B, &mut ops, true);
        }
        // op signextend_w
        {
            let mut ops: Vec<[u64; 2]> = Vec::new();
            // mid_box: a >= 0x804DB000 && a < 0x804DC000 && (a & 7) == 0 && b == 0
            for a in (0x804DB000..0x804DC000).step_by(8) {
                for b in 0..1 {
                    ops.push([a, b]);
                }
            }
            self.table.add_ops(ZiskOp::SIGNEXTEND_W, &mut ops, true);
        }
        // op rev8
        {
            let mut ops: Vec<[u64; 2]> = Vec::new();
            // low_rect: a == 0 && b < 4096
            for a in 0..1 {
                for b in 0..4096 {
                    ops.push([a, b]);
                }
            }
            self.table.add_ops(ZiskOp::REV8, &mut ops, true);
        }
        // op pack
        {
            let mut ops: Vec<[u64; 2]> = Vec::new();
            // low_rect: a < 519 && b < 2
            for a in 0..519 {
                for b in 0..2 {
                    ops.push([a, b]);
                }
            }
            // mid_box: a >= 0xFFFFF007 && a < 0x100000007 && (a & 7) == 7 && b == 0
            for a in (0xFFFFF007..0x100000007).step_by(8) {
                for b in 0..1 {
                    ops.push([a, b]);
                }
            }
            self.table.add_ops(ZiskOp::PACK, &mut ops, true);
        }
        // op pack_h
        {
            let mut ops: Vec<[u64; 2]> = Vec::new();
            // low_rect: a < 256 && b < 256
            for a in 0..256 {
                for b in 0..256 {
                    ops.push([a, b]);
                }
            }
            self.table.add_ops(ZiskOp::PACK_H, &mut ops, true);
        }
        // op pack_w
        {
            let mut ops: Vec<[u64; 2]> = Vec::new();
            // low_rect: a < 5 && b == 0
            for a in 0..5 {
                for b in 0..1 {
                    ops.push([a, b]);
                }
            }
            self.table.add_ops(ZiskOp::PACK_W, &mut ops, true);
        }
        // op ror
        {
            let mut ops: Vec<[u64; 2]> = Vec::new();
            // low_rect: a == 0 && b < 39
            for a in 0..1 {
                for b in 0..39 {
                    ops.push([a, b]);
                }
            }
            // mid_box: a >= 0x3AF7879156A36007 && a < 0x3AF7879156A37007 && (a & 7) == 7 && b == 38
            for a in (0x3AF7879156A36007..0x3AF7879156A37007).step_by(8) {
                for b in 0x26..0x27 {
                    ops.push([a, b]);
                }
            }
            // mid_box: a >= 0x98F726F10695F006 && a < 0x98F726F106960006 && (a & 7) == 6 && b == 38
            for a in (0x98F726F10695F006..0x98F726F106960006).step_by(8) {
                for b in 0x26..0x27 {
                    ops.push([a, b]);
                }
            }
            // mid_box: a >= 0xDAD4333928708005 && a < 0xDAD4333928709005 && (a & 7) == 5 && b == 38
            for a in (0xDAD4333928708005..0xDAD4333928709005).step_by(8) {
                for b in 0x26..0x27 {
                    ops.push([a, b]);
                }
            }
            self.table.add_ops(ZiskOp::ROR, &mut ops, true);
        }
        // op ror_w
        {
            let mut ops: Vec<[u64; 2]> = Vec::new();
            // low_rect: a == 0 && b < 20
            for a in 0..1 {
                for b in 0..20 {
                    ops.push([a, b]);
                }
            }
            self.table.add_ops(ZiskOp::ROR_W, &mut ops, true);
        }
        // op clz
        {
            let mut ops: Vec<[u64; 2]> = Vec::new();
            // low_rect: a == 0 && b < 4096
            for a in 0..1 {
                for b in 0..4096 {
                    ops.push([a, b]);
                }
            }
            self.table.add_ops(ZiskOp::CLZ, &mut ops, true);
        }
        // op ctz
        {
            let mut ops: Vec<[u64; 2]> = Vec::new();
            // low_rect: a == 0 && b < 129
            for a in 0..1 {
                for b in 0..129 {
                    ops.push([a, b]);
                }
            }
            self.table.add_ops(ZiskOp::CTZ, &mut ops, true);
        }
        // op cpop_w
        {
            let mut ops: Vec<[u64; 2]> = Vec::new();
            // low_rect: a == 0 && b < 32
            for a in 0..1 {
                for b in 0..32 {
                    ops.push([a, b]);
                }
            }
            self.table.add_ops(ZiskOp::CPOP_W, &mut ops, true);
        }
        // op bclr
        {
            let mut ops: Vec<[u64; 2]> = Vec::new();
            // low_rect: a == 0 && b < 64
            for a in 0..1 {
                for b in 0..64 {
                    ops.push([a, b]);
                }
            }
            self.table.add_ops(ZiskOp::BCLR, &mut ops, true);
        }
        // op bext
        {
            let mut ops: Vec<[u64; 2]> = Vec::new();
            // low_rect: a < 201 && b < 8
            for a in 0..201 {
                for b in 0..8 {
                    ops.push([a, b]);
                }
            }
            // mid_box: a >= 0x3F0006 && a < 0x3F1006 && (a & 7) == 6 && b >= 1 && b < 17
            for a in (0x3F0006..0x3F1006).step_by(8) {
                for b in 0x1..0x11 {
                    ops.push([a, b]);
                }
            }
            self.table.add_ops(ZiskOp::BEXT, &mut ops, true);
        }
        // op binv
        {
            let mut ops: Vec<[u64; 2]> = Vec::new();
            // mid_box: a >= 0x8000000000000000 && a < 0x8000000000001000 && b == 63
            for a in 0x8000000000000000..0x8000000000001000 {
                for b in 0x3F..0x40 {
                    ops.push([a, b]);
                }
            }
            self.table.add_ops(ZiskOp::BINV, &mut ops, true);
        }
        // op bset
        {
            let mut ops: Vec<[u64; 2]> = Vec::new();
            // low_rect: a < 69 && b < 128
            for a in 0..69 {
                for b in 0..128 {
                    ops.push([a, b]);
                }
            }
            // mid_box: a >= 0xC71E8005 && a < 0xC71E9005 && (a & 7) == 5 && b == 32
            for a in (0xC71E8005..0xC71E9005).step_by(8) {
                for b in 0x20..0x21 {
                    ops.push([a, b]);
                }
            }
            self.table.add_ops(ZiskOp::BSET, &mut ops, true);
        }
    }

    #[inline(always)]
    pub fn is_frequent_op(op: u8, a: u64, b: u64) -> bool {
        match op {
            ZiskOp::SLL => {
                a < 4096 && b < 161
                    || a >= 4096 && a < 0x10000 && (a & 7) == 0 && b < 243
                    || a >= 0xFFFFF007 && a < 0x100000007 && (a & 7) == 7 && b < 225
                    || a >= 0xFFFFFFFFFEFEF007 && a < 0xFFFFFFFFFEFF0007 && (a & 7) == 7 && b == 32
            }
            ZiskOp::SRL => {
                a < 4096 && b < 65
                    || a >= 0xFFFFFFFFFFF000
                        && a < 0x100000000001000
                        && (a & 7) == 0
                        && b >= 1
                        && b < 121
                    || a >= 0x1102000000000000
                        && a < 0x1102000000001000
                        && (a & 7) == 0
                        && b >= 8
                        && b < 57
                    || a >= 0xFFFFFFFF00000000
                        && a < 0xFFFFFFFF00001000
                        && (a & 7) == 0
                        && b >= 8
                        && b < 57
            }
            ZiskOp::SRA => {
                a < 4096 && b < 64
                    || a >= 4096 && a < 24576 && b >= 3 && b < 6
                    || a >= 0x6000000000000000 && a < 0x6000000000001000 && (a & 7) == 0 && b == 56
                    || a >= 0x6100000000000000 && a < 0x6100000000001000 && (a & 7) == 0 && b == 56
            }
            ZiskOp::SRL_W => {
                a < 530 && b < 32
                    || a >= 0xFFFFA007 && a < 0x100000007 && (a & 7) == 7 && b == 24
                    || a >= 0x11020000000000 && a < 0x11020000001000 && (a & 7) == 0 && b == 24
                    || a >= 0x1102000000000000 && a < 0x1102000000001000 && (a & 7) == 0 && b == 24
            }
            ZiskOp::SRA_W => a < 25 && b < 32,
            ZiskOp::SIGNEXTEND_B => a == 0 && b < 256,
            ZiskOp::SIGNEXTEND_W => a >= 0x804DB000 && a < 0x804DC000 && (a & 7) == 0 && b == 0,
            ZiskOp::REV8 => a == 0 && b < 4096,
            ZiskOp::PACK => {
                a < 519 && b < 2 || a >= 0xFFFFF007 && a < 0x100000007 && (a & 7) == 7 && b == 0
            }
            ZiskOp::PACK_H => a < 256 && b < 256,
            ZiskOp::PACK_W => a < 5 && b == 0,
            ZiskOp::ROR => {
                a == 0 && b < 39
                    || a >= 0x3AF7879156A36007 && a < 0x3AF7879156A37007 && (a & 7) == 7 && b == 38
                    || a >= 0x98F726F10695F006 && a < 0x98F726F106960006 && (a & 7) == 6 && b == 38
                    || a >= 0xDAD4333928708005 && a < 0xDAD4333928709005 && (a & 7) == 5 && b == 38
            }
            ZiskOp::ROR_W => a == 0 && b < 20,
            ZiskOp::CLZ => a == 0 && b < 4096,
            ZiskOp::CTZ => a == 0 && b < 129,
            ZiskOp::CPOP_W => a == 0 && b < 32,
            ZiskOp::BCLR => a == 0 && b < 64,
            ZiskOp::BEXT => {
                a < 201 && b < 8
                    || a >= 0x3F0006 && a < 0x3F1006 && (a & 7) == 6 && b >= 1 && b < 17
            }
            ZiskOp::BINV => a >= 0x8000000000000000 && a < 0x8000000000001000 && b == 63,
            ZiskOp::BSET => {
                a < 69 && b < 128 || a >= 0xC71E8005 && a < 0xC71E9005 && (a & 7) == 5 && b == 32
            }
            _ => false,
        }
    }

    #[inline(always)]
    pub fn get_row(op: u8, a: u64, b: u64) -> usize {
        let relative_offset = match op {
            ZiskOp::SLL => {
                if a < 4096 && b < 161 {
                    (a * 161 + b) as usize
                } else if a >= 4096 && a < 0x10000 && (a & 7) == 0 && b < 243 {
                    (((a - 0x1000) / 8) * 243 + b) as usize + 659456
                } else if a >= 0xFFFFF007 && a < 0x100000007 && (a & 7) == 7 && b < 225 {
                    (((a - 0xFFFFF007) / 8) * 225 + b) as usize + 2525696
                } else if a >= 0xFFFFFFFFFEFEF007
                    && a < 0xFFFFFFFFFEFF0007
                    && (a & 7) == 7
                    && b == 32
                {
                    ((a - 0xFFFFFFFFFEFEF007) / 8) as usize + 2640896
                } else {
                    Self::NO_FROPS
                }
            }
            ZiskOp::SRL => {
                if a < 4096 && b < 65 {
                    (a * 65 + b) as usize
                } else if a >= 0xFFFFFFFFFFF000
                    && a < 0x100000000001000
                    && (a & 7) == 0
                    && b >= 1
                    && b < 121
                {
                    (((a - 0xFFFFFFFFFFF000) / 8) * 120 + (b - 0x1)) as usize + 266240
                } else if a >= 0x1102000000000000
                    && a < 0x1102000000001000
                    && (a & 7) == 0
                    && b >= 8
                    && b < 57
                {
                    (((a - 0x1102000000000000) / 8) * 49 + (b - 0x8)) as usize + 389120
                } else if a >= 0xFFFFFFFF00000000
                    && a < 0xFFFFFFFF00001000
                    && (a & 7) == 0
                    && b >= 8
                    && b < 57
                {
                    (((a - 0xFFFFFFFF00000000) / 8) * 49 + (b - 0x8)) as usize + 414208
                } else {
                    Self::NO_FROPS
                }
            }
            ZiskOp::SRA => {
                if a < 4096 && b < 64 {
                    (a * 64 + b) as usize
                } else if a >= 4096 && a < 24576 && b >= 3 && b < 6 {
                    ((a - 0x1000) * 3 + (b - 0x3)) as usize + 262144
                } else if a >= 0x6000000000000000
                    && a < 0x6000000000001000
                    && (a & 7) == 0
                    && b == 56
                {
                    ((a - 0x6000000000000000) / 8) as usize + 323584
                } else if a >= 0x6100000000000000
                    && a < 0x6100000000001000
                    && (a & 7) == 0
                    && b == 56
                {
                    ((a - 0x6100000000000000) / 8) as usize + 324096
                } else {
                    Self::NO_FROPS
                }
            }
            ZiskOp::SRL_W => {
                if a < 530 && b < 32 {
                    (a * 32 + b) as usize
                } else if a >= 0xFFFFA007 && a < 0x100000007 && (a & 7) == 7 && b == 24 {
                    ((a - 0xFFFFA007) / 8) as usize + 16960
                } else if a >= 0x11020000000000 && a < 0x11020000001000 && (a & 7) == 0 && b == 24 {
                    ((a - 0x11020000000000) / 8) as usize + 20032
                } else if a >= 0x1102000000000000
                    && a < 0x1102000000001000
                    && (a & 7) == 0
                    && b == 24
                {
                    ((a - 0x1102000000000000) / 8) as usize + 20544
                } else {
                    Self::NO_FROPS
                }
            }
            ZiskOp::SRA_W => {
                if a < 25 && b < 32 {
                    (a * 32 + b) as usize
                } else {
                    Self::NO_FROPS
                }
            }
            ZiskOp::SIGNEXTEND_B => {
                if a == 0 && b < 256 {
                    (a * 256 + b) as usize
                } else {
                    Self::NO_FROPS
                }
            }
            ZiskOp::SIGNEXTEND_W => {
                if a >= 0x804DB000 && a < 0x804DC000 && (a & 7) == 0 && b == 0 {
                    ((a - 0x804DB000) / 8) as usize
                } else {
                    Self::NO_FROPS
                }
            }
            ZiskOp::REV8 => {
                if a == 0 && b < 4096 {
                    (a * 4096 + b) as usize
                } else {
                    Self::NO_FROPS
                }
            }
            ZiskOp::PACK => {
                if a < 519 && b < 2 {
                    (a * 2 + b) as usize
                } else if a >= 0xFFFFF007 && a < 0x100000007 && (a & 7) == 7 && b == 0 {
                    ((a - 0xFFFFF007) / 8) as usize + 1038
                } else {
                    Self::NO_FROPS
                }
            }
            ZiskOp::PACK_H => {
                if a < 256 && b < 256 {
                    (a * 256 + b) as usize
                } else {
                    Self::NO_FROPS
                }
            }
            ZiskOp::PACK_W => {
                if a < 5 && b == 0 {
                    a as usize
                } else {
                    Self::NO_FROPS
                }
            }
            ZiskOp::ROR => {
                if a == 0 && b < 39 {
                    (a * 39 + b) as usize
                } else if a >= 0x3AF7879156A36007
                    && a < 0x3AF7879156A37007
                    && (a & 7) == 7
                    && b == 38
                {
                    ((a - 0x3AF7879156A36007) / 8) as usize + 39
                } else if a >= 0x98F726F10695F006
                    && a < 0x98F726F106960006
                    && (a & 7) == 6
                    && b == 38
                {
                    ((a - 0x98F726F10695F006) / 8) as usize + 551
                } else if a >= 0xDAD4333928708005
                    && a < 0xDAD4333928709005
                    && (a & 7) == 5
                    && b == 38
                {
                    ((a - 0xDAD4333928708005) / 8) as usize + 1063
                } else {
                    Self::NO_FROPS
                }
            }
            ZiskOp::ROR_W => {
                if a == 0 && b < 20 {
                    (a * 20 + b) as usize
                } else {
                    Self::NO_FROPS
                }
            }
            ZiskOp::CLZ => {
                if a == 0 && b < 4096 {
                    (a * 4096 + b) as usize
                } else {
                    Self::NO_FROPS
                }
            }
            ZiskOp::CTZ => {
                if a == 0 && b < 129 {
                    (a * 129 + b) as usize
                } else {
                    Self::NO_FROPS
                }
            }
            ZiskOp::CPOP_W => {
                if a == 0 && b < 32 {
                    (a * 32 + b) as usize
                } else {
                    Self::NO_FROPS
                }
            }
            ZiskOp::BCLR => {
                if a == 0 && b < 64 {
                    (a * 64 + b) as usize
                } else {
                    Self::NO_FROPS
                }
            }
            ZiskOp::BEXT => {
                if a < 201 && b < 8 {
                    (a * 8 + b) as usize
                } else if a >= 0x3F0006 && a < 0x3F1006 && (a & 7) == 6 && b >= 1 && b < 17 {
                    (((a - 0x3F0006) / 8) * 16 + (b - 0x1)) as usize + 1608
                } else {
                    Self::NO_FROPS
                }
            }
            ZiskOp::BINV => {
                if a >= 0x8000000000000000 && a < 0x8000000000001000 && b == 63 {
                    (a - 0x8000000000000000) as usize
                } else {
                    Self::NO_FROPS
                }
            }
            ZiskOp::BSET => {
                if a < 69 && b < 128 {
                    (a * 128 + b) as usize
                } else if a >= 0xC71E8005 && a < 0xC71E9005 && (a & 7) == 5 && b == 32 {
                    ((a - 0xC71E8005) / 8) as usize + 8832
                } else {
                    Self::NO_FROPS
                }
            }
            _ => return Self::NO_FROPS,
        };
        if relative_offset == Self::NO_FROPS {
            Self::NO_FROPS
        } else {
            relative_offset + OP_TABLE_OFFSETS[op as usize - OP_TABLE_OFFSETS_START]
        }
    }

    #[inline(always)]
    pub fn count(&self) -> usize {
        self.table.count()
    }

    #[cfg(test)]
    pub fn test_table_offsets(&mut self) {
        self.build_table();
        let (start, offsets) = self.table.generate_table_offsets();
        if (start != OP_TABLE_OFFSETS_START) || (offsets != OP_TABLE_OFFSETS) {
            self.table.print_table_offsets();
            panic!("Table offsets do not match expected values");
        }
        assert_eq!(start, OP_TABLE_OFFSETS_START);
        assert_eq!(offsets, OP_TABLE_OFFSETS);
    }

    #[inline(always)]
    pub fn generate_full_table(&self) -> Vec<(u8, u64, u64, u64, bool)> {
        self.table.generate_full_table()
    }

    #[inline(always)]
    pub fn generate_table(&self) -> Vec<(u8, u64, u64)> {
        self.table.generate_table()
    }

    #[inline(always)]
    pub fn generate_cmd(
        &mut self,
        cmd_name: &'static str,
        default_file: &'static str,
    ) -> Result<(), Box<dyn Error>> {
        self.build_table();
        let full_table = self.generate_full_table();
        let full_table_count = full_table.len();
        self.table.generate_cmd(
            "Zisk",
            "BinaryExtensionFrops",
            cmd_name,
            default_file,
            full_table,
            full_table_count,
        )
    }
}

#[test]
fn test_table_offsets() {
    let mut fops = BinaryExtensionFrops::new();
    fops.test_table_offsets();
}

#[test]
fn test_all_accessible_values() {
    let mut fops = BinaryExtensionFrops::new();
    fops.build_table();
    let table = fops.generate_full_table();
    FrequentOpsHelpers::test_all_accessible_values(
        &table,
        BinaryExtensionFrops::is_frequent_op,
        BinaryExtensionFrops::get_row,
    );
}
