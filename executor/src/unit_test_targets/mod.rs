...
pub fn build_manager_registry(
    ...
    if let Some(b) = bundle.binary_sm() {
    ...
    if let Some(a) = bundle.arith_sm() {
        map.insert(ARITH_AIR_IDS[0], erase(a.arith_full_sm()));
    }
    if let Some(m) = bundle.mem_sm() {
    ...
    if let Some(k) = bundle.keccakf_sm() {
        map.insert(KECCAKF_AIR_IDS[0], erase(k.keccakf_sm()));
    }
    if let Some(s) = bundle.sha256f_sm() {
        map.insert(SHA_256_F_AIR_IDS[0], erase(s.sha256f_sm()));
    }
    if let Some(p) = bundle.poseidon2_sm() {
        map.insert(POSEIDON_2_AIR_IDS[0], erase(p.poseidon2_sm()));
    }
    if let Some(b) = bundle.blake2_sm() {
        map.insert(BLAKE_2_BR_AIR_IDS[0], erase(b.blake2_sm()));
    }
    if let Some(a) = bundle.arith_eq_sm() {
        map.insert(ARITH_EQ_AIR_IDS[0], erase(a.arith_eq_sm()));
    }
    if let Some(a) = bundle.arith_eq_384_sm() {
        map.insert(ARITH_EQ_384_AIR_IDS[0], erase(a.arith_eq_384_sm()));
    }
    if let Some(a) = bundle.add256_sm() {
        map.insert(ADD_256_AIR_IDS[0], erase(a.add256_sm()));
    }

    if let Some(d) = bundle.dma_sm() {
        ...
        map.insert(DMA_64_ALIGNED_INPUT_CPY_AIR_IDS[0], erase(d.dma_64_aligned_inputcpy_sm()));
