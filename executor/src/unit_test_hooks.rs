...
impl UnitTestHookBag {
        ...
        S: UnitTestSm<Goldilocks>,
    {
        ...
        assert_eq!(
            std::mem::size_of::<S::Row>(),
            row_size * std::mem::size_of::<Goldilocks>(),
            "UnitTestHookBag: row layout mismatch for air_id {} — \
             size_of::<S::Row>() != ROW_SIZE * size_of::<Goldilocks>()",
            S::air_id(),
        );
        ...
        let erased: ErasedHook = Box::new(move |row_idx, raw| {
            ...
            //   - `size_of::<S::Row>() == row_size * size_of::<Goldilocks>()`,
            //     asserted at registration above, so the cast reads exactly
            //     `raw`'s bytes — no over-read.
