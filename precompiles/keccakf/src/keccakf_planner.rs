//! The `KeccakfPlanner` module defines a planner for generating execution plans specific to
...
/// The `KeccakfPlanner` struct organizes execution plans for arithmetic instances and tables.
///
/// It allows adding metadata about instances and tables and generates plans
/// based on the provided counters.
#[derive(Default)]
pub struct KeccakfPlanner {
...
impl KeccakfPlanner {
    /// Creates a new `KeccakfPlanner`.
    ///
    /// # Returns
    /// A new `KeccakfPlanner` instance with no preconfigured instances or tables.
    pub fn new() -> Self {
    ...
    ///
    /// # Returns
    /// The updated `KeccakfPlanner` instance.
...
impl Planner for KeccakfPlanner {
    ...
    fn plan(&self, counters: Vec<(ChunkId, Box<dyn BusDeviceMetrics>)>) -> Vec<Plan> {
        ...
        counters.iter().for_each(|(chunk_id, counter)| {
            let reg_counter = Metrics::as_any(&**counter).downcast_ref::<KeccakfCounter>().unwrap();
