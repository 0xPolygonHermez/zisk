...
struct Emitter {
    uint64_t* out;
    // Words of one row are `stride` apart: 1 for row-major, the air's row count for
    // column-major, where `out` points at the row's slot in word 0's column.
    size_t stride;
    uint64_t carry;
    int bits;
    int idx;
...
__device__ __forceinline__ void emit(Emitter& e, uint64_t v, int n) {
    ...
    if (total >= 64) {
        e.out[(size_t)(e.idx++) * e.stride] = e.carry;
...
__device__ __forceinline__ void emit_row(uint64_t* __restrict__ row,
                                         ...
                                         uint64_t rc_bits,
                                         uint64_t step_addr) {
    Emitter e{row, stride, flags, 4, 0};
...
#pragma unroll
    ...
    if (e.bits != 0) {
        row[(size_t)e.idx * stride] = e.carry;
    }
}
...
#pragma unroll
    ...
    // Column-major: `out + r` is row r's slot in word 0's column and consecutive words of
    // that row are dst_rows apart, so a base is a ROW INDEX rather than a word offset.
...
#pragma unroll
        ...
        if (in_range && owns) {
            ...
            emit_row(slot_out + (size_t)(GROUP_IN_A + k) * ROW_WORDS, emit_stride, flags, a_in, zeros, 0, 0,
                     false, zeros, zeros, zeros, zeros, 0, step_addr);
            emit_row(slot_out + (size_t)(GROUP_IN_B + k) * ROW_WORDS, emit_stride, flags, b_in, zeros, 0, 0,
...
#pragma unroll
            ...
            const bool owns = ((k & 1) == 0) == is_a;
            if (in_range && owns) {
                emit_row(grp + (size_t)(k) * ROW_WORDS, emit_stride, flags, sa, sb, last ? 0 : pa[k],
...
#pragma unroll
        ...
        const bool owns = ((k & 1) == 0) == is_a;
        if (in_range && owns) {
            emit_row(slot_out + (size_t)(GROUP_OUT_A + k) * ROW_WORDS, emit_stride, flags, a_out, zeros, 0, 0,
                     false, zeros, zeros, zeros, zeros, 0, 0);
            emit_row(slot_out + (size_t)(GROUP_OUT_B + k) * ROW_WORDS, emit_stride, flags, b_out, zeros, 0, 0,
                     false, zeros, zeros, zeros, zeros, 0, 0);
        }
    }
}
...
/// One thread per operation, so a slot's two ops are adjacent lanes.
static int keccakf_launch(const KeccakfGpuOp* d_ops, uint32_t num_ops, uint64_t* d_dst,
    ...
    keccakf_witness_kernel<<<grid, block, 0, stream>>>(d_ops, num_ops, num_slots, d_dst);
    return 0;
}

// available / out_words / fill / fill_probe. 128 is the width the published
// timings were measured at; 256 is the kernel's __launch_bounds__.
ZISK_GPU_WITNESS_ENTRIES(keccakf, KeccakfGpuOp, 128, 256, keccakf_out_words, keccakf_launch)

/// Benchmark driver: allocates, uploads, times and downloads on its own.
///
/// Used only by the CPU/GPU comparison test — the prover path goes through
/// [`keccakf_gpu_fill`], which owns none of this. `out_host`, when non-null,
/// receives the packed trace (num_slots · CLOCKS · ROW_WORDS u64) and the
/// download is timed separately. The device buffer is poisoned with 0xAA before
/// the first launch, so any word the kernel fails to write shows up as a
/// mismatch rather than as an accidental zero.
extern "C" int keccakf_gpu_witness(const KeccakfGpuOp* ops,
    ...
    // Same constraint the shared entry points enforce: whole warps only, since
    // slot pairs exchange across neighbouring lanes under the full mask.
    ...
    ZISK_CUDA_TRY(cudaEventCreate(&e0));
    ZISK_CUDA_TRY(cudaEventCreate(&e1));

    ZISK_CUDA_TRY(cudaEventRecord(e0));
    ...
    ZISK_CUDA_TRY(cudaMemset(d_out, 0xAA, out_bytes));
    ZISK_CUDA_TRY(cudaEventRecord(e1));
    ZISK_CUDA_TRY(cudaEventSynchronize(e1));
    ZISK_CUDA_TRY(cudaEventElapsedTime(&ms, e0, e1));
    t->alloc_ms = ms;

    ZISK_CUDA_TRY(cudaHostAlloc(&h_ops, in_bytes, cudaHostAllocDefault));
    memcpy(h_ops, ops, in_bytes);
    ZISK_CUDA_TRY(cudaEventRecord(e0));
    ZISK_CUDA_TRY(cudaMemcpy(d_ops, h_ops, in_bytes, cudaMemcpyHostToDevice));
    ZISK_CUDA_TRY(cudaEventRecord(e1));
    ZISK_CUDA_TRY(cudaEventSynchronize(e1));
    ZISK_CUDA_TRY(cudaEventElapsedTime(&ms, e0, e1));
    t->h2d_ms = ms;

    {
        ...
        for (uint32_t i = 0; i < iters; ++i) {
            ...
            ZISK_CUDA_TRY(cudaEventRecord(e1));
            ZISK_CUDA_TRY(cudaEventSynchronize(e1));
            ZISK_CUDA_TRY(cudaGetLastError());
            ZISK_CUDA_TRY(cudaEventElapsedTime(&ms, e0, e1));
    ...
    if (out_host != nullptr) {
        ZISK_CUDA_TRY(cudaHostAlloc(&h_out, out_bytes, cudaHostAllocDefault));
        ZISK_CUDA_TRY(cudaEventRecord(e0));
        ZISK_CUDA_TRY(cudaMemcpy(h_out, d_out, out_bytes, cudaMemcpyDeviceToHost));
        ZISK_CUDA_TRY(cudaEventRecord(e1));
        ZISK_CUDA_TRY(cudaEventSynchronize(e1));
        ZISK_CUDA_TRY(cudaEventElapsedTime(&ms, e0, e1));
        t->d2h_ms = ms;
