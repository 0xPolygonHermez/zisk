...
impl<F: PrimeField64> DmaStrategy<F> {
    ...
    const DMA_ROWS: usize = DmaTrace::<DmaTrace<F>>::NUM_ROWS;
    const DMA_MEMCPY_ROWS: usize = DmaMemCpyTrace::<DmaMemCpyTrace<F>>::NUM_ROWS;
    const DMA_INPUTCPY_ROWS: usize = DmaInputCpyTrace::<DmaInputCpyTrace<F>>::NUM_ROWS;
    const DMA_PRE_POST_ROWS: usize = DmaPrePostTrace::<DmaPrePostTrace<F>>::NUM_ROWS;
    const DMA_PRE_POST_MEMCPY_ROWS: usize =
        DmaPrePostMemCpyTrace::<DmaPrePostMemCpyTrace<F>>::NUM_ROWS;
    const DMA_PRE_POST_INPUTCPY_ROWS: usize =
        DmaPrePostInputCpyTrace::<DmaPrePostInputCpyTrace<F>>::NUM_ROWS;
    const DMA_64_ALIGNED_ROWS: usize = Dma64AlignedTrace::<Dma64AlignedTrace<F>>::NUM_ROWS;
    const DMA_64_ALIGNED_MEMCPY_ROWS: usize =
        Dma64AlignedMemCpyTrace::<Dma64AlignedMemCpyTrace<F>>::NUM_ROWS;
    const DMA_64_ALIGNED_MEMSET_ROWS: usize =
        Dma64AlignedMemSetTrace::<Dma64AlignedMemSetTrace<F>>::NUM_ROWS;
    const DMA_64_ALIGNED_INPUTCPY_ROWS: usize =
        Dma64AlignedInputCpyTrace::<Dma64AlignedInputCpyTrace<F>>::NUM_ROWS;
    const DMA_64_ALIGNED_MEM_ROWS: usize =
        Dma64AlignedMemTrace::<Dma64AlignedMemTrace<F>>::NUM_ROWS;
    const DMA_UNALIGNED_ROWS: usize = DmaUnalignedTrace::<DmaUnalignedTrace<F>>::NUM_ROWS;
    // Dma
    // DmaMemCpy
    // DmaInputCpy
