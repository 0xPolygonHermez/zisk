...
use zisk_pil::{
    DmaInputCpyTrace, DmaInputCpyTraceRow, DmaInputCpyTraceRowOps, DmaInputCpyTraceRowPacked,
    DMA_ROM_ID,
};
...
impl<F: PrimeField64> DmaInputCpySM<F> {
    ...
    fn compute_witness_inner<R: DmaInputCpyTraceRowOps<F> + Copy + Send>(
        ...
        let mut trace = DmaInputCpyTrace::<R>::new_from_vec_zeroes(trace_buffer)?;
