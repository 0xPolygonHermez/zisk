...
use super::{BLOCK_SIZE_BITS, BLOCK_SIZE_BYTES};

#[derive(Debug)]
pub struct Sha256Input<'a> {
    input: &'a [u8],
    offset: usize,
    padding: [u8; BLOCK_SIZE_BYTES + 8],
    padding_size: usize,
...
impl<'a> Sha256Input<'a> {
    ...
    pub fn new(input: &'a [u8]) -> Self {
        ...
        let remaining_bytes = input_size % BLOCK_SIZE_BYTES;
        let mut padding = [0u8; BLOCK_SIZE_BYTES + 8];
        let mut padding_size = BLOCK_SIZE_BYTES - remaining_bytes;
        if remaining_bytes >= 56 {
            // If remaining bytes are equal or bigger than 56 bytes, we need to pad to the next block
            padding_size += BLOCK_SIZE_BYTES;
        }

        // Set the first byte to 0x80
        padding[0] = 0x80;

        // Store message length in (big-endian) bits in last 8 bytes
        u64_to_bytes((input_size * 8) as u64, &mut padding[padding_size - 8..], true);
    ...
    /// Reads next BLOCK_SIZE_BYTES bytes of input
    pub fn get_next(&mut self, buffer: &mut [u8; BLOCK_SIZE_BYTES]) -> bool {
        match self.state {
            State::ReadingInput => {
                if self.offset + BLOCK_SIZE_BYTES <= self.input.len() {
                    // Full block of input
                    buffer
                        .copy_from_slice(&self.input[self.offset..self.offset + BLOCK_SIZE_BYTES]);
                    self.offset += BLOCK_SIZE_BYTES;
                    true
                } else {
                    ...
                    let pad_to_copy = BLOCK_SIZE_BYTES - remaining;
            ...
            State::ReadingPadding => {
                ...
                if self.padding_offset < self.padding_size {
                    buffer.copy_from_slice(
                        &self.padding[self.padding_offset..self.padding_offset + BLOCK_SIZE_BYTES],
                    );
    ...
    pub fn get_next_bits(&mut self, bit_buffer: &mut [u8; BLOCK_SIZE_BITS]) -> bool {
        let mut byte_buffer = [0u8; BLOCK_SIZE_BYTES];
...
fn u64_to_bytes(input: u64, output: &mut [u8], big_endian: bool) {
    assert!(output.len() >= 8, "Output buffer too small");
    if big_endian {
        output[..8].copy_from_slice(&input.to_be_bytes());
    } else {
        output[..8].copy_from_slice(&input.to_le_bytes());
    }
}
...
mod tests {
    ...
    fn test_empty_input() {
        ...
        assert!(input.padding_size == BLOCK_SIZE_BYTES);

        assert!(input.get_next(&mut buffer));
        assert_eq!(buffer[0], 0x80);
        for i in 1..BLOCK_SIZE_BYTES {
    ...
    fn test_small_input() {
        ...
        for i in 56..BLOCK_SIZE_BYTES - 2 {
            assert_eq!(buffer[i], 0x00); // Padding
        }
        assert_eq!(buffer[BLOCK_SIZE_BYTES - 2], 0x01);
        assert_eq!(buffer[BLOCK_SIZE_BYTES - 1], 0xB8);
    ...
    fn test_small_input2() {
        ...
        for i in 57..BLOCK_SIZE_BYTES {
            assert_eq!(buffer[i], 0x00); // Original data
        }

        assert!(input.get_next(&mut buffer));
        // 56*8 in binary is 0b00000001 0b11000000 == 0x01 || 0xC0
        for i in 0..BLOCK_SIZE_BYTES - 2 {
            assert_eq!(buffer[i], 0x00); // Padding
        }
        assert_eq!(buffer[BLOCK_SIZE_BYTES - 2], 0x01);
        assert_eq!(buffer[BLOCK_SIZE_BYTES - 1], 0xC0);
    ...
    fn test_full_input() {
        ...
        let data = vec![0xFFu8; BLOCK_SIZE_BYTES];
        let mut input = Sha256Input::new(&data);
        let mut buffer = [0u8; BLOCK_SIZE_BYTES];
        assert!(input.padding_size == BLOCK_SIZE_BYTES);
        ...
        for i in 1..BLOCK_SIZE_BYTES - 2 {
            assert_eq!(buffer[i], 0x00); // Padding
        }
        assert_eq!(buffer[BLOCK_SIZE_BYTES - 2], 0x02);
        assert_eq!(buffer[BLOCK_SIZE_BYTES - 1], 0x00);
    ...
    fn test_big_input() {
        ...
        let data = vec![0xFFu8; BLOCK_SIZE_BYTES + 57];
        let mut input = Sha256Input::new(&data);
        let mut buffer = [0u8; BLOCK_SIZE_BYTES];
        ...
        assert_eq!(buffer[BLOCK_SIZE_BYTES - 2], 0x03);
        assert_eq!(buffer[BLOCK_SIZE_BYTES - 1], 0xC8);
