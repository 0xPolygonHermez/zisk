//! The `Sha256fPlanner` module defines a planner for generating execution plans specific to
...
/// The `Sha256fPlanner` struct organizes execution plans for arithmetic instances and tables.
///
/// It allows adding metadata about instances and tables and generates plans
/// based on the provided counters.
#[derive(Default)]
pub struct Sha256fPlanner {
...
impl Sha256fPlanner {
    /// Creates a new `Sha256fPlanner`.
    ///
    /// # Returns
    /// A new `Sha256fPlanner` instance with no preconfigured instances or tables.
    pub fn new() -> Self {
    ...
    ///
    /// # Returns
    /// The updated `Sha256fPlanner` instance.
...
impl Planner for Sha256fPlanner {
    ...
    fn plan(&self, counters: Vec<(ChunkId, Box<dyn BusDeviceMetrics>)>) -> Vec<Plan> {
        ...
        counters.iter().for_each(|(chunk_id, counter)| {
            let reg_counter =
                Metrics::as_any(&**counter).downcast_ref::<Sha256fCounterInputGen>().unwrap();
