...
use zisk_pil::{Blake2sTrace, Blake2sTraceRow, Blake2sTraceRowOps};
...
/// Number of unconditional rot-12 XOR table lookups per row: vb', 4 bytes.
...
pub struct Blake2sSM<F: PrimeField64> {
    ...
    /// Number of available blake2s permutations in the trace.
...
impl<F: PrimeField64> Blake2sSM<F> {
    /// Creates a new Blake2s State Machine instance.
    ///
    /// # Returns
    /// A new `Blake2sSM` instance.
    pub fn new(std: Arc<Std<F>>) -> Arc<Self> {
        // Compute some useful values
        let num_available_blake2s = Blake2sTrace::<Blake2sTraceRow<F>>::NUM_ROWS / CLOCKS;
    ...
    pub fn process_input<R: Blake2sTraceRowOps<F>>(
        ...
        for (k, row) in trace.iter_mut().enumerate().take(CLOCKS) {
            ...
            for (i, z1_byte) in z1.to_le_bytes().into_iter().enumerate() {
                row.set_vb_prime_s(i, 0, (z1_byte & 0x0F) << 4);
                row.set_vb_prime_s(i, 1, z1_byte >> 4);
            }
            ...
            for i in 0..4 {
                let rows = [
                    ...
                    BlakeTableSM::calculate_table_row(vb_bytes[i], vc_p_bytes[i], 12),
    ...
    pub fn compute_witness<R: Blake2sTraceRowOps<F>>(
        &self,
        _sctx: &SetupCtx<F>,
        inputs: &[Vec<Blake2sInput>],
        trace_buffer: Vec<F>,
    ) -> ProofmanResult<AirInstance<F>> {
        let mut trace = Blake2sTrace::<R>::new_from_vec_zeroes(trace_buffer)?;
        ...
        // Split trace into per-operation cycles for parallel processing
        let flat_inputs: Vec<&Blake2sInput> = inputs.iter().flatten().collect();
