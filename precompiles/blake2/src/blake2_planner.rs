//! The `Blake2Planner` module defines a planner for generating execution plans specific to
...
use crate::Blake2CounterInputGen;
...
/// The `Blake2Planner` struct organizes execution plans for arithmetic instances and tables.
///
/// It allows adding metadata about instances and tables and generates plans
/// based on the provided counters.
#[derive(Default)]
pub struct Blake2Planner {
...
impl Blake2Planner {
    /// Creates a new `Blake2Planner`.
    ///
    /// # Returns
    /// A new `Blake2Planner` instance with no preconfigured instances or tables.
    pub fn new() -> Self {
    ...
    ///
    /// # Returns
    /// The updated `Blake2Planner` instance.
...
impl Planner for Blake2Planner {
    ...
    fn plan(&self, counters: Vec<(ChunkId, Box<dyn BusDeviceMetrics>)>) -> Vec<Plan> {
        ...
        counters.iter().for_each(|(chunk_id, counter)| {
            let reg_counter =
                Metrics::as_any(&**counter).downcast_ref::<Blake2CounterInputGen>().unwrap();
