//! The `ArithEq384Planner` module defines a planner for generating execution plans specific to
...
use crate::ArithEq384CounterInputGen;
...
/// The `ArithEq384Planner` struct organizes execution plans for arithmetic instances and tables.
///
/// It allows adding metadata about instances and tables and generates plans
/// based on the provided counters.
#[derive(Default)]
pub struct ArithEq384Planner {
...
impl ArithEq384Planner {
    /// Creates a new `ArithEq384Planner`.
    ///
    /// # Returns
    ...
    ///
    /// # Returns
    /// The updated `ArithEq384Planner` instance.
...
impl Planner for ArithEq384Planner {
    ...
    fn plan(&self, counters: Vec<(ChunkId, Box<dyn BusDeviceMetrics>)>) -> Vec<Plan> {
        ...
        counters.iter().for_each(|(chunk_id, counter)| {
            let reg_counter =
                Metrics::as_any(&**counter).downcast_ref::<ArithEq384CounterInputGen>().unwrap();
