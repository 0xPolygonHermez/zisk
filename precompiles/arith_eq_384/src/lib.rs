...
zisk_precompile! {
    ...
    op_type = ArithEq384,
    row = ArithEq384Trace,
    // The same air at three heights. They prove the same operations and commit the same columns, so
    // the planner sizes them together: the tall ones keep the instance count down, the short ones
