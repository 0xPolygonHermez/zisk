//! The `Arith256Planner` module defines a planner for generating execution plans specific to
...
impl Arith256Planner {
    ...
    ///
    /// # Returns
    /// The updated `Arith256Planner` instance.
...
impl Planner for Arith256Planner {
    ...
    fn plan(&self, counters: Vec<(ChunkId, Box<dyn BusDeviceMetrics>)>) -> Vec<Plan> {
        ...
        counters.iter().for_each(|(chunk_id, counter)| {
            let reg_counter =
                Metrics::as_any(&**counter).downcast_ref::<Arith256CounterInputGen>().unwrap();
