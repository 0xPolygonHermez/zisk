...
use crate::equations;
use ark_ff::Field;
pub use ark_secp256k1::Fq as Secp256k1Field;
...
impl Secp256k1 {
    ...
    #[inline(always)]
    pub fn slope_denominator(is_dbl: bool, p1: &[u64; 8], p2: &[u64; 8]) -> Secp256k1Field {
        if is_dbl {
            let y1 = Secp256k1Field::from(ark_ff::BigInt::<4>(p1[4..8].try_into().unwrap()));
            y1 + y1
        } else {
            Secp256k1Field::from(ark_ff::BigInt::<4>(p2[0..4].try_into().unwrap()))
                - Secp256k1Field::from(ark_ff::BigInt::<4>(p1[0..4].try_into().unwrap()))
        }
    }
    ...
    #[inline(always)]
    fn slope_inv(is_dbl: bool, p1: &[u64; 8], p2: &[u64; 8]) -> Secp256k1Field {
        Self::slope_denominator(is_dbl, p1, p2)
            .inverse()
            .expect("Secp256k1: slope denominator is zero")
    }
    ...
    fn prepare_with_inv(
        ...
        let s = if is_dbl {
            (Secp256k1Field::from(3u64) * x1 * x1) * den_inv
        } else {
            (y2 - y1) * den_inv
        };
        let x3 = s * s - (x1 + x2);
