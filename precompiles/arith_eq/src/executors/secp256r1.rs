...
use crate::equations;
use ark_ff::Field;
pub use ark_secp256r1::Fq as Secp256r1Field;

const COLS: u8 = 32;

lazy_static! {
    pub static ref SECP256R1_PRIME: BigInt = BigInt::parse_bytes(
    ...
    pub static ref SECP256R1_Q2_OFFSET: BigInt = BigInt::from(1) << 257;
}
...
impl Secp256r1 {
    ...
    fn point_from_8x64(p: &[u64; 8]) -> (Secp256r1Field, Secp256r1Field) {
        (
            Secp256r1Field::from(ark_ff::BigInt::<4>(p[0..4].try_into().unwrap())),
            Secp256r1Field::from(ark_ff::BigInt::<4>(p[4..8].try_into().unwrap())),
        )
    }
    ...
    #[inline(always)]
    pub fn slope_denominator(is_dbl: bool, p1: &[u64; 8], p2: &[u64; 8]) -> Secp256r1Field {
        if is_dbl {
            let y1 = Secp256r1Field::from(ark_ff::BigInt::<4>(p1[4..8].try_into().unwrap()));
            y1 + y1
        } else {
            Secp256r1Field::from(ark_ff::BigInt::<4>(p2[0..4].try_into().unwrap()))
                - Secp256r1Field::from(ark_ff::BigInt::<4>(p1[0..4].try_into().unwrap()))
        }
    }
    ...
    #[inline(always)]
    fn slope_inv(is_dbl: bool, p1: &[u64; 8], p2: &[u64; 8]) -> Secp256r1Field {
        Self::slope_denominator(is_dbl, p1, p2)
            .inverse()
            .expect("Secp256r1: slope denominator is zero")
    }
    ...
    fn prepare_with_inv(
        ...
        let s = if is_dbl {
            (Secp256r1Field::from(3u64) * x1 * x1 + Secp256r1Field::from(-3)) * den_inv
        } else {
            (y2 - y1) * den_inv
        };
        let x3 = s * s - (x1 + x2);
        ...
        let q0 = if is_dbl {
            ...
            debug_assert!((&_q0 % &*SECP256R1_PRIME).is_zero());
            &*SECP256R1_DBL_Q0_OFFSET - (&_q0 / &*SECP256R1_PRIME)
        } else {
            let _q0: BigInt = &s * (&x2 - &x1) - &y2 + &y1;
            debug_assert!((&_q0 % &*SECP256R1_PRIME).is_zero());
            (&_q0 / &*SECP256R1_PRIME) + &*SECP256R1_ADD_Q0_OFFSET
        };

        let _q1 = &s * &s - &x1 - &x2 - &x3;
        debug_assert!((&_q1 % &*SECP256R1_PRIME).is_zero());
        let q1 = (&_q1 / &*SECP256R1_PRIME) + &*SECP256R1_Q1_OFFSET;

        let _q2 = &s * &x1 - &s * &x3 - &y1 - &y3;
        debug_assert!((&_q2 % &*SECP256R1_PRIME).is_zero());
        let q2 = &*SECP256R1_Q2_OFFSET - (&_q2 / &*SECP256R1_PRIME);
    ...
    pub fn execute_add_dbl_with_inv(
        ...
        for icol in 0..COLS {
            ...
            data.eq[index] = [
                if is_dbl {
                    equations::Secp256r1Dbl::calculate(icol, &data.x1, &data.y1, &data.s, &data.q0)
                } else {
                    equations::Secp256r1Add::calculate(
                        icol, &data.x1, &data.y1, &data.x2, &data.y2, &data.s, &data.q0,
                    )
                },
                equations::Secp256r1X3::calculate(
                    icol, &data.x1, &data.x2, &data.x3, &data.s, &data.q1,
                ),
                equations::Secp256r1Y3::calculate(
            ...
            for ieq in 0..3 {
                ...
                debug_assert!(
                    ...
                    "EqSecp256r1 residue eq{ieq} ({index}) #:{value} cin:{cin}"
                );
            }
        }
        data
    ...
    pub fn verify_add_dbl(is_dbl: bool, p1: &[u64; 8], p2: &[u64; 8], p: &[u64; 8]) {
        ...
        data.check_ranges();
        let op = if is_dbl { "Secp256r1Dbl" } else { "Secp256r1Add" };
        for i in 0..2 {
            let offset = (i + 1) * 4 - 1;
