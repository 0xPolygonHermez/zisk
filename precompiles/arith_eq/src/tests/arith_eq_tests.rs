//! Guest-emulation tests for the ArithEq precompile ops (moved out of `lib.rs`).
...
#[serial]
fn arith256_tests() {
    ELF_ARITH256.run_emulation(ZiskStdin::new(), None).expect("arith256 guest emulation failed");
}
...
#[serial]
fn bn254_add_tests() {
    ELF_BN254_ADD.run_emulation(ZiskStdin::new(), None).expect("bn254_add guest emulation failed");
}

#[test]
#[serial]
fn bn254_dbl_tests() {
    ELF_BN254_DBL.run_emulation(ZiskStdin::new(), None).expect("bn254_dbl guest emulation failed");
}
