...
/// Work that fills whole instances goes to the air that needs the fewest of them, and the least
/// memory breaks a tie. With every `Large` air the same height both halves of the criterion show up
/// at once: the short airs lose on the instance count, and between the two equally capacious tall
/// ones the specialized (narrower) air wins on memory.
#[test]
fn a_bulk_goes_where_the_fewest_instances_are_needed() {
    let tall = meta_of(Arith256XLargeTrace::<()>::AIR_ID);
    let cap_tall = cap(&tall);
    ...
    // Its own short sibling is half the height, so it would need twice the instances — ruled out
    // before memory is ever compared.
    let short = meta_of(Arith256XTrace::<()>::AIR_ID);
    assert!((3 * cap_tall).div_ceil(cap(&short)) > 3);

    // The universal tall air is exactly as capacious, so it needs the same three instances. What
    // rules it out is the tie-break: it is wider, and so dearer.
    let universal = meta_of(ArithEqLargeTrace::<()>::AIR_ID);
    assert_eq!(cap(&universal), cap_tall, "the ladders are level, so this really is a tie");
