...
impl ExpressionManager {
    pub fn new(config: ExpressionManagerConfig) -> Self {
        ...
        for i in 0..sin_count {
            exprs.push(Expression::input(i, Some(config.in_prefix.clone())));
    ...
    pub fn mark_end_of_round(&mut self, round: usize, output_dir: &Path) -> std::io::Result<()> {
        ...
        writeln!(writer)?;
        writeln!(writer, "round_resets = {};", round_events.len())?;
        writeln!(writer)?;
        ...
        for (i, event) in round_events.iter().enumerate() {
            if let Some(old_expr) = self.get_expression(event.old_expr_id) {
                ...
                writeln!(writer, "{}[{}] = {};", self.config.out_prefix, i, old_expr)?;
                writeln!(writer)?;
            }
        }
    ...
    fn create_reset_expression(
        ...
        let new_expr =
            Expression::reset(new_reset_id, original_degree, Some(self.config.in_prefix.clone()));
        let new_degree = new_expr.degree();
        let new_max_value = new_expr.max_value();
    ...
    pub fn generate_summary_file(&self, output_dir: &Path) -> std::io::Result<()> {
        ...
        if !self.max_values_by_round.is_empty() {
            ...
            for round in 0..self.max_values_by_round.len() {
                let round_events: Vec<_> =
                    self.expr_events.iter().filter(|e| e.round == Some(round)).collect();
