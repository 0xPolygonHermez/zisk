...
pub use pin::{Pin, PinId, PinSource, NUM_PINS};
pub use utils::*;
...
mod tests {
    ...
    fn test_one_gate_circuits() {
        ...
        for (i, input) in inputs.iter().enumerate() {
            ...
            state.xor2(state.sin_refs[0], PinId::A, state.sin_refs[1], PinId::A, free_ref);
        ...
        for (i, input) in inputs.iter().enumerate() {
            ...
            // Perform the circuit operation
            let free_ref = state.get_free_ref();
            state.nand(state.sin_refs[0], PinId::A, state.sin_refs[1], PinId::A, free_ref);
    ...
    fn test_simple_circuit() {
        ...
        for (i, input) in inputs.iter().enumerate() {
            ...
            state.xor2(free_ref1, PinId::D, state.sin_refs[1], PinId::A, free_ref2);
