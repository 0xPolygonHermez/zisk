...
main() {
    ...
    cached=0
    if [[ "${USE_CACHE_SETUP}" == "1" && -n "${CACHE_SETUP_HASH:-}" ]]; then
        info "CACHE_SETUP_HASH is set (${CACHE_SETUP_HASH}), skipping build steps..."
        cache_setup_folder="${OUTPUT_DIR}/${PLATFORM}/${CACHE_SETUP_HASH}"
        cached=1
    fi
    ...
    fi # end: skip build steps if CACHE_SETUP_HASH is set
