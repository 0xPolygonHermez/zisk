...
#   starkstructs_path    print the starkstructs settings file for $HASH_MODE
#
# Variables this reads (defaulted if unset):
#   HASH_MODE                hash mode (Poseidon1 | Poseidon2 | blake3)
...
# auto-detected hasCompressor flags back into this file, so each mode
# accumulates its own.
starkstructs_path() {
  case "$HASH_MODE" in
...
# Hash mode the proving key under $1 was built with, from its pilout.globalInfo.json,
# or empty when there is no such key yet.
#
# $HASH_MODE is what the NEXT build would use; this is what an EXISTING key already is. The two
...
# is already on disk has to ask the key. Never guessed: a key that records no mode is an
...
proving_key_hash_mode() {
  ...
  local mode
  mode="$(jq -r '.hash // empty' "$gi")" || { echo "failed to parse $gi" >&2; return 2; }
  [ -n "$mode" ] || { echo "no 'hash' field in $gi — rebuild the setup so it records its hash mode" >&2; return 2; }
  printf '%s' "$mode"
}
...
compute_input_hash() (
  ...
  echo "hashing $(wc -l < "$pil_list") .pil files + $(starkstructs_path) + ${#fixed_bins[@]} *_fixed.bin + tool refs" >&2
  {
    ...
    cat "${fixed_bins[@]}"
    # The mode is a build input, not just a flag: it picks the starkstructs
