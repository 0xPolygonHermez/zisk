...
//
// Output layout (nPublicsAgg = 6, from chain.toml). Inputs are full-width
// (ZISK_PUBLICS()); the generator zero-fills the [nPublicsAgg, 64) tail.
//   [0]                     chain endpoint `old`
//   [1]                     chain endpoint `new`
//   [2..nPublicsAgg) Poseidon digest of [1,2,3,free_inputs[0]] (NormalizePublics)
template AggregatePublics(nFreeInputs, nPublicsAgg) {
    ...
    // Both sides must agree on the normalized digest slots [2..nPublicsAgg).
    // On leaves the digest is derived from identical tuples, so they match; on
    // aggregated proofs the digest was propagated unchanged, so it still matches.
    for (var i = 2; i < nPublicsAgg; i++) {
