...
template NormalizePublics(nPublics, nFreeInputs) {
    ...
    // aggregated fold reads back the same array it fed a leaf's NormalizePublics.
    ...
    // Drain the unused digest tail so Circom doesn't complain about unused
    // signals (free inputs are all consumed by free_outputs above).
    for (var i = 4; i < 8; i++) {
        _ <== digest[i];
    }
}
