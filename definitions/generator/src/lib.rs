...
fn generated_note(regen_cmd: &str) -> String {
    format!("@generated by `{regen_cmd}` \u{2014} do not edit.")
}
...
pub fn write(
    ...
    for (target, dir, ext) in dirs.each() {
        write_dir(&files, target, dir, ext)?;
    }
    Ok(())
}
...
pub fn check(
    ...
    for (target, dir, ext) in dirs.each() {
        check_dir(&files, target, dir, ext, &mut problems);
    }
