    ...
    inline void loadPolynomials(StepsParams& params, ParserArgs &parserArgs, std::vector<Dest> &dests, __m256i *bufferT_, uint64_t row, uint64_t domainSize) {
        ...
        for(uint64_t k = 0; k < constPolsUsed.size(); ++k) {
            ...
            for(uint64_t o = 0; o < nOpenings; ++o) {
                ...
                Goldilocks::load_avx(bufferT_[nColsStagesAcc[(setupCtx.starkInfo.nStages + 2)*o] + k], &bufferT[nrowsPack*o]);
            }
        }
                ...
                for(uint64_t k = 0; k < dests[j].params.size(); ++k) {
                    ...
                    for (uint64_t kk = 0; kk < dests[j].params[k].parserParams.nOps; ++kk) {
                        switch (ops[kk]) {
                            ...
                            case 30: {
                                // OPERATION WITH DEST: tmp3 - SRC0: airvalue - SRC1: commit1
                            ...
                            case 39: {
                                // OPERATION WITH DEST: tmp3 - SRC0: commit3 - SRC1: airvalue
                            ...
                            case 44: {
                                // OPERATION WITH DEST: tmp3 - SRC0: tmp3 - SRC1: airvalue
                            ...
                            case 47: {
                                // OPERATION WITH DEST: tmp3 - SRC0: challenge - SRC1: airvalue
                            ...
                            case 49: {
                                // OPERATION WITH DEST: tmp3 - SRC0: airgroupvalue - SRC1: airvalue
                                Goldilocks3::op_avx(args[i_args], tmp3[args[i_args + 1]], airgroupValues[args[i_args + 2]], airValues[args[i_args + 3]]);
                                i_args += 4;
                                break;
                            }
                            case 50: {
                                // OPERATION WITH DEST: tmp3 - SRC0: airvalue - SRC1: airvalue
