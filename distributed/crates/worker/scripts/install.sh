...
#   --proving-key PATH Path to the proving key directory (default: /var/lib/zisk-worker/provingKey)
...
#   1. Creates the 'zisk-worker' system user
#   2. Installs the binary to /usr/local/bin/zisk-worker
#   3. Installs config to /etc/zisk/worker.toml
#   4. Creates working directories (/var/lib/zisk-worker, /var/log/zisk)
#   5. Writes /etc/systemd/system/zisk-worker.service
#   6. Runs: systemctl daemon-reload && systemctl enable --now zisk-worker
...
BINARY_SRC=""
CONFIG_SRC=""
PROVING_KEY="${WORK_DIR}/provingKey"
NO_START=false
NO_ENABLE=false
UNINSTALL=false
...
if $UNINSTALL; then
    ...
    echo "Done. Config and data directories are left in place."
    echo "Remove manually if no longer needed:"
    echo "  sudo rm -rf ${CONFIG_DIR} ${WORK_DIR} ${LOG_DIR}"
    exit 0
fi
...
if [[ -z "${BINARY_SRC}" ]]; then
    ...
    cargo build --release -p zisk-worker --manifest-path "${WORKSPACE_ROOT}/Cargo.toml"
...
if [[ -n "${CONFIG_SRC}" ]]; then
    ...
    SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
    SAMPLE="${SCRIPT_DIR}/../config/prod.toml"
    if [[ -f "${SAMPLE}" ]]; then
...
ExecStart=${BINARY_DST} --config ${CONFIG_DST} --proving-key ${PROVING_KEY}
Restart=on-failure
RestartSec=5
