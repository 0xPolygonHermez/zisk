...
impl tonic::IntoRequest<RegisterRecurserAggregatorRequest> for RegisterRecurserAggregatorRequestDto {
...
impl TryFrom<RegisterRecurserAggregatorRequest> for RegisterRecurserAggregatorRequestDto {
    ...
    fn try_from(req: RegisterRecurserAggregatorRequest) -> std::result::Result<Self, Self::Error> {
        let spec = req
            .spec
            .ok_or_else(|| "register_recurser_aggregator.spec must be set".to_string())?;
...
impl TryFrom<JobKind> for DomainJobKind {
    ...
    fn try_from(kind: JobKind) -> std::result::Result<Self, Self::Error> {
        ...
        match inner {
            ...
            job_kind::Kind::SetupAggregator(r) => Ok(DomainJobKind::SetupAggregator(
                DomainSetupAggregatorRequest { recurser_id: r.recurser_id },
            )),
