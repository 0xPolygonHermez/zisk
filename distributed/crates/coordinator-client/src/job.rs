...
impl Job {
    pub(crate) fn new(job_id: String, client: CoordinatorClient) -> Result<Self> {
        let job_id = Uuid::parse_str(&job_id)
            .with_context(|| format!("coordinator returned invalid job_id UUID: {job_id}"))?;
    ...
    /// Spawn a background task that watches job events via the coordinator stream.
    ///
