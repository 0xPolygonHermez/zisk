...
impl ZiskCoordinatorApi for CoordinatorGrpc {
    ...
    async fn cancel_job(
        ...
        let job_id = JobId::from(inner.job_id);
        let reason = inner.reason.unwrap_or_else(|| "Cancelled by operator".to_string());
...
/// Implements the internal cluster protocol (coordinator ↔ worker streaming).
#[tonic::async_trait]
impl ZiskClusterApi for CoordinatorGrpc {
