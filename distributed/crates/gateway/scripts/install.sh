#!/usr/bin/env bash
# install.sh — install zisk-gateway as a systemd service on Linux.
#
# Usage:
...
#   --config PATH    Install an existing gateway.toml instead of the sample
...
#   1. Creates the 'zisk-gateway' system user
#   2. Installs the binary to /usr/local/bin/zisk-gateway
#   3. Installs config to /etc/zisk/gateway.toml
#   4. Creates the /var/lib/zisk working directory
#   5. Writes /etc/systemd/system/zisk-gateway.service
#   6. Runs: systemctl daemon-reload && systemctl enable --now zisk-gateway

set -euo pipefail

BINARY_NAME="zisk-gateway"
BINARY_DST="/usr/local/bin/${BINARY_NAME}"
CONFIG_DIR="/etc/zisk"
CONFIG_DST="${CONFIG_DIR}/gateway.toml"
...
if [[ -z "${BINARY_SRC}" ]]; then
    ...
    cargo build --release -p zisk-gateway --manifest-path "${WORKSPACE_ROOT}/Cargo.toml"
...
if [[ -n "${CONFIG_SRC}" ]]; then
    ...
    SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
    SAMPLE="${SCRIPT_DIR}/../config/gateway.example.toml"
    if [[ -f "${SAMPLE}" ]]; then
...
cat > "${UNIT_FILE}" <<EOF
[Unit]
Description=ZisK Gateway — public API gateway for the ZisK proving system
