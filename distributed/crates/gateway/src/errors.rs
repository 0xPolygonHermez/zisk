//! Gateway error types with stable API error codes and gRPC status mapping.
//!
//! Every [`GatewayError`] variant maps to a stable numeric code matching the
//! API reference in `book/developer/gateway_api.md`. Clients that embed the
...
/// Stable numeric error codes (matches `book/developer/gateway_api.md`).
pub mod codes {
...
impl From<GatewayError> for Status {
    fn from(err: GatewayError) -> Self {
        ...
        if let GatewayError::Internal(ref detail) = err {
            tracing::error!(
                error_code = err.code(),
                error_name = err.name(),
                detail,
...
pub fn internal(msg: impl std::fmt::Display) -> GatewayError {
...
mod tests {
    ...
    fn error_codes_are_stable() {
        assert_eq!(GatewayError::JobNotFound(Uuid::new_v4()).code(), 1001);
    ...
    fn internal_error_returns_generic_status_message() {
        let status = Status::from(GatewayError::Internal("secret detail".into()));
    ...
    fn not_found_errors_map_to_not_found_code() {
        assert_eq!(Status::from(GatewayError::ProgramNotFound("h".into())).code(), Code::NotFound);
        assert_eq!(Status::from(GatewayError::JobNotFound(Uuid::new_v4())).code(), Code::NotFound);
    }
}
